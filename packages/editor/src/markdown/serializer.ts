...
import {
  isPortableAttachmentId,
  serializePortableAttachmentUrl,
  ...
  _serializer = new MarkdownSerializer(
    {
      ...
      image(state, node) {
        // A picture out of the session's attachments folder travels by its
        // id (markdown/portable-attachment-id.ts), never by a local URL.
        const attachmentId = node.attrs.attachmentId;
        state.write(
          serializeMarkdownImage({
            src: isPortableAttachmentId(attachmentId)
              ? serializePortableAttachmentUrl(attachmentId)
              : node.attrs.src,
            alt: node.attrs.alt,
      ...
      fileAttachment(state, node) {
        ...
        const attachmentId = node.attrs.attachmentId;
        const source = isPortableAttachmentId(attachmentId)
          ? serializePortableAttachmentUrl(attachmentId)
          : node.attrs.src || "";
