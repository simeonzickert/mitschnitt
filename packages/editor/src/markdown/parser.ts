...
import {
  parsePortableAttachmentUrl,
  PORTABLE_ATTACHMENT_PREFIX,
} from "./portable-attachment-id";
...
function fileAttachmentPlugin(md: MarkdownIt) {
  md.block.ruler.before(
    ...
    (
      ...
      const isPortableUrl = line.startsWith(
        PORTABLE_ATTACHMENT_PREFIX,
        urlStart,
      );
      ...
      const attachmentId = isPortableUrl
        ? parsePortableAttachmentUrl(url)
        : null;
      // A portable link whose id is not a plain file name (a path, `..`) is
      // no attachment; leave the line to the ordinary link parser.
      if (isPortableUrl && attachmentId === null) {
        return false;
      }
  ...
  _parser = new MarkdownParser(markdownSchema, md, {
    ...
    image: {
      ...
      getAttrs: (tok) => {
        ...
        // `attachment://<id>` is the picture's id, not a URL to keep.
        const attachmentId = parsePortableAttachmentUrl(src);
        return {
