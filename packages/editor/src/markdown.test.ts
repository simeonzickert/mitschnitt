...
describe("fileAttachment round-trip", () => {
  ...
  // G6a (Opus, Fix-Runde 2). An uploaded picture is stored with its
  // attachmentId and NO src (normalizePortableAttachmentUrls strips the
  // asset: URL before the save), so json2md wrote `![alt]()` and md2json
  // came back with attachmentId null -- every path that rewrites a note
  // through markdown (session proposals, chat corrections) lost the picture,
  // and since the attachment reconciler exists, its file with it. Now the
  // image travels as `attachment://<id>` like a file attachment does, with
  // the id percent-encoded (ids are file names, and a space ends a markdown
  // link destination).
  test("round-trips an image's attachment id without a local URL", () => {
    const attachmentId = 'diagram 1 "final".png';
    const markdown = json2md({
      type: "doc",
      content: [
        {
          type: "image",
          attrs: { attachmentId, alt: "diagram" },
        },
      ],
    });

    expect(markdown).toBe(
      '![diagram](attachment://diagram%201%20%22final%22.png "char-editor-width=80")',
    );
    const image = md2json(markdown).content?.[0];
    expect(image?.type).toBe("image");
    expect(image?.attrs?.attachmentId).toBe(attachmentId);
    expect(image?.attrs?.src).toBeNull();
    expect(image?.attrs?.alt).toBe("diagram");
  });

  test("round-trips a file attachment whose id is a file name, not a uuid", () => {
    const attachmentId = "report 2026.pdf";
    const markdown = json2md({
      type: "doc",
      content: [
        {
    ...
    expect(markdown).toBe("[report.pdf](attachment://report%202026.pdf)");
    const attachment = md2json(markdown).content?.[0];
    expect(attachment?.type).toBe("fileAttachment");
    expect(attachment?.attrs?.attachmentId).toBe(attachmentId);
    expect(attachment?.attrs?.src).toBeNull();
  ...
  test("does not accept a path as a portable image id", () => {
    for (const bad of [
      "![x](attachment://../a.png)",
      "![x](attachment://..%2Fa.png)",
      "![x](attachment://sub/a.png)",
      "![x](attachment://..)",
    ]) {
      const image = md2json(bad).content?.[0];
      expect(image?.attrs?.attachmentId ?? null, bad).toBeNull();
    }
  });
      ...
      content: [
        {
          ...
          attrs: {
            ...
            src: "asset://localhost/%2FUsers%2Fsungbin%2FLibrary%2FApplication%20Support%2Fcom.anarlog.dev%2Fsessions%2Ff515cc6f%2Fattachments%2FCE2%20The%20devil%20wears%20Prada%20script%202.pdf",
          },
        },
        {
          type: "fileAttachment",
          attrs: {
            ...
            src: "asset://localhost/%2FUsers%2Fsungbin%2FLibrary%2FApplication%20Support%2Fcom.anarlog.dev%2Fsessions%2Ff515cc6f%2Fattachments%2F2021-13630%20%E1%84%8C%E1%85%A9%E1%84%89%E1%85%A5%E1%86%BC%E1%84%87%E1%85%B5%E1%86%AB%20%E1%84%86%E1%85%AE%E1%86%AF%E1%84%85%E1%85%B5%E1%84%92%E1%85%A1%E1%86%A8%201%20HW2.pdf",
          },
        },
      ],
    };
