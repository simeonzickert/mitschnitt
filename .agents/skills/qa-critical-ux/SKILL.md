...
> **Mitschnitt-Fork (02.09.2026):** Die Release-Pipeline des Originals (`desktop_cd.yaml`, `desktop_publish.yaml`, `desktop_store_publish.yaml`, CrabNebula-Provenance) existiert in diesem Fork nicht mehr. Der eigene Weg ist `scripts/mitschnitt-deploy.sh` (bauen, mit des Betreibers Developer-ID signieren, installieren). Die Schritte unten, die diese Workflows nennen, sind Upstream-Historie und laufen hier nicht. **`scripts/run-native-dev-qa.sh` ist stillgelegt** (meldet sich und beendet mit Exit 2): es baute „Anarlog Dev.app" gegen `desktop_cd.yaml` und die Endpunkte des Originals. Die QA-Fragen selbst (Start, Onboarding, Aufnahme, Zusammenfassung) gelten weiter -- auf der per `scripts/mitschnitt-deploy.sh` installierten App.
