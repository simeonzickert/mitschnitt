...
# Mitschnitt-Fork (02.09.2026): im Fork ohne Release-Pipeline stillgelegt.
# Dieses Skript baute "Anarlog Dev.app" gegen die Endpunkte aus
# .github/workflows/desktop_cd.yaml (Supabase, App-/API-URL des Originals)
# und pruefte die Kandidaten-Provenienz dieser Pipeline. Der Workflow ist im
# Fork geloescht; der eigene Weg ist scripts/mitschnitt-deploy.sh (bauen,
# signieren, installieren). Der Rest der Datei bleibt als Upstream-Referenz
# lesbar, laeuft aber nicht mehr -- er wuerde an der ersten Pruefung
# ("Desktop release workflow not found") sterben.

set -euo pipefail
umask 077

echo "run-native-dev-qa.sh: im Mitschnitt-Fork stillgelegt (keine Release-Pipeline, desktop_cd.yaml existiert nicht). Bauen und installieren: scripts/mitschnitt-deploy.sh" >&2
exit 2
