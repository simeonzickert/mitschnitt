...
struct FloatingBarView: View {
  ...
  private var expandedPanel: some View {
    ...
    return VStack(spacing: FloatingBarLayout.hoverHandleGap) {
      ...
      ZStack(alignment: .topTrailing) {
        VStack(spacing: 12) {
          ...
          if let trackWarningMessage {
            trackWarningBanner(trackWarningMessage)
    ...
    .animation(.easeOut(duration: 0.12), value: isBarHovered)
    .animation(.easeOut(duration: 0.12), value: model.silentTracks)
  }
  ...
  private func audioControl(width: CGFloat, height: CGFloat) -> some View {
    ...
    return Button(action: { performClick(RustBridge.stopListening) }) {
      Group {
        ...
        } else if model.hasTrackLevels {
          DualTrackBars(
            levels: model.trackLevels,
            silentTracks: model.silentTracks,
            color: accentColor,
      ...
      .overlay(alignment: .topTrailing) {
        if trackWarningMessage != nil, !isStopHovered {
    ...
    .buttonStyle(.plain)
    .accessibilityLabel(
      trackWarningMessage.map { "Stop listening. \($0)" } ?? "Stop listening"
    )
    .onHover { isStopHovered = $0 }
    .animation(.easeOut(duration: 0.12), value: model.silentTracks)
  }

  /// Plain-language warning for the compact glyph and the expanded row, or nil
  /// when there is nothing to say.
  private var trackWarningMessage: String? {
    guard model.hasTrackLevels, model.status != .error else { return nil }
    return FloatingTrackHealth.describe(silentTracks: model.silentTracks)
  }

  private func trackWarningBanner(_ message: String) -> some View {
    let shape = RoundedRectangle(
      cornerRadius: FloatingBarLayout.controlCornerRadius,
      style: .continuous
    )
    ...
    .background(shape.fill(warningColor.opacity(0.14)))
    .overlay(shape.strokeBorder(warningColor.opacity(0.34), lineWidth: 0.5))
  ...
  /// Amber, deliberately not the red accent: a dead track is a warning about
  /// the recording, not the recording's own colour turning up louder.
  private var warningColor: Color {
    if model.colorScheme == .dark {
      return Color(red: 1, green: 0.76, blue: 0.28)
    }

    return Color(red: 0.72, green: 0.44, blue: 0.02)
  }
...
private struct DancingBars: View {
  ...
  /// A flat track is drawn flat. No wave, no drive -- the bars sit on the
  /// baseline so a dead channel reads as dead at a glance, not as "quiet".
  var isFlat: Bool = false

  private let minHeight: CGFloat = 4
  private let maxHeight: CGFloat = 20

  static func width(barCount: Int, barWidth: CGFloat, barSpacing: CGFloat) -> CGFloat {
    CGFloat(barCount) * barWidth + CGFloat(max(barCount - 1, 0)) * barSpacing
  }

  var body: some View {
    TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: isFlat)) { timeline in
...
/// Microphone and system audio as two separate waveforms.
///
/// The whole point of the Kanal-tot-Waechter is that one number cannot show a
/// dead track. So the bar stops showing one number: the left group is the
/// microphone, the right group is the other side, and a group that has gone
/// silent turns amber and lies flat.
///
/// It occupies exactly the same 26pt the single waveform did, so nothing in the
/// pill's geometry moves.
private struct DualTrackBars: View {
  let levels: FloatingTrackLevels
  let silentTracks: [FloatingTrack]
  let color: Color
  ...
  private let groupSpacing: CGFloat = 5

  var body: some View {
    HStack(spacing: groupSpacing) {
      trackBars(for: .mic)
      trackBars(for: .speaker)
    }
    .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .center)
  }

  private func trackBars(for track: FloatingTrack) -> some View {
    let isSilent = silentTracks.contains(track)

    return DancingBars(
      color: isSilent ? warningColor : color,
      amplitude: levels.level(for: track),
      barCount: barCount,
      barWidth: barWidth,
    ...
    .frame(
      width: DancingBars.width(
        barCount: barCount, barWidth: barWidth, barSpacing: barSpacing)
    )
    .accessibilityLabel(track == .mic ? "Microphone level" : "Other side level")
  }
}
