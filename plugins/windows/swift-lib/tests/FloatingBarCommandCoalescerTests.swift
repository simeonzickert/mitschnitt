...
final class FloatingBarCommandCoalescerTests: XCTestCase {
  ...
  func testAppliesLatestVisibilityAndUpdateInCallOrder() {
    ...
    let coalescer = FloatingBarCommandCoalescer(
      ...
      apply: { action in
        ...
        case .trackLevels: actionNames.append("trackLevels")
        case .resetTrackWatch: actionNames.append("resetTrackWatch")
        }
      })
  ...
  func testCoalescesTrackLevelsByPeakSoASpikeIsNeverLost() {
    let scheduler = ManualScheduler()
    var actions: [FloatingBarCommandCoalescer.Action] = []
    let coalescer = FloatingBarCommandCoalescer(
      scheduler: scheduler.schedule,
    ...
    // A burst where each track peaks in a different reading, and the LAST
    // reading is flat on both. "Latest wins" would report silence on both.
    coalescer.enqueueTrackLevels(FloatingTrackLevels(mic: 0.6, speaker: 0))
    coalescer.enqueueTrackLevels(FloatingTrackLevels(mic: 0, speaker: 0.4))
    coalescer.enqueueTrackLevels(FloatingTrackLevels(mic: 0, speaker: 0))

    scheduler.runNext()

    XCTAssertEqual(actions.count, 1)
    guard case .trackLevels(let levels) = actions.first else {
      return XCTFail("Expected coalesced track levels")
    }
    XCTAssertEqual(levels.mic, 0.6)
    XCTAssertEqual(levels.speaker, 0.4)
  }

  func testResetDropsLevelsObservedBeforeItAndKeepsLaterOnes() {
    let scheduler = ManualScheduler()
    var actions: [FloatingBarCommandCoalescer.Action] = []
    let coalescer = FloatingBarCommandCoalescer(
      scheduler: scheduler.schedule,
    ...
    coalescer.enqueueTrackLevels(FloatingTrackLevels(mic: 0.9, speaker: 0.9))
    coalescer.enqueueResetTrackWatch()
    coalescer.enqueueTrackLevels(FloatingTrackLevels(mic: 0.1, speaker: 0))

    scheduler.runNext()

    XCTAssertEqual(actions.count, 2)
    guard case .resetTrackWatch = actions.first else {
      return XCTFail("Expected the reset to be applied before the new levels")
    }
    guard case .trackLevels(let levels) = actions.last else {
      return XCTFail("Expected the post-reset levels")
    }
    XCTAssertEqual(levels.mic, 0.1)
    XCTAssertEqual(levels.speaker, 0)
  }
