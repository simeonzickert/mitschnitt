...
pub async fn send_test(
    ...
    let body = Bytes::from(envelope(
        EVENT_TEST,
        serde_json::json!({ "message": "This is a test delivery from Mitschnitt." }),
    ));
...
async fn deliver_once(
    ...
    // NICHT UMBENENNEN: Diese Kopfzeilen gehen nach draussen an eine vom
    // Nutzer eingetragene Gegenstelle, die genau auf diese Namen filtert und
    // die Signatur darueber prueft. Ein Umbenennen ist ein Vertragsbruch
    // gegenueber jedem bereits eingerichteten Empfaenger, ohne dass in der App
    // irgendetwas sichtbar besser wird.
    let response = client
        .post(&endpoint.url)
