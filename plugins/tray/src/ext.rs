...
use crate::{
    schedule::{TrayAgendaSection, TrayScheduleEvent, agenda_sections, next_schedule_refresh_ms},
...
pub fn build_app_menu(app: &AppHandle<tauri::Wry>) -> Result<Menu<tauri::Wry>> {
    ...
    // Kein Hilfe-Menue: "Report Bug" und "Suggest Feature" oeffneten beide
    // https://anarlog.so/discord, den Community-Kanal des Originals. Dieser
    // Fork hat keinen oeffentlichen Kanal; ein Menue, das ins Leere fuehrt,
...
impl<'a, M: tauri::Manager<tauri::Wry>> Tray<'a, tauri::Wry, M> {
    ...
    // Entscheid des Eigentuemers 22.09.2026: kein Text mehr neben dem
    // Tray-Icon. Diese Methode wird aktuell von niemandem aufgerufen (dead
    // API), ignoriert den uebergebenen Titel aber bewusst, statt ihn
    // durchzureichen -- sonst koennte ein spaeterer Aufrufer versehentlich
    // wieder Text in die Menueleiste schreiben, ohne dass diese Regel
    // auffaellt.
    pub fn set_title(&self, _title: Option<&str>) -> Result<()> {
        let app = self.manager.app_handle();
        if let Some(tray) = app.tray_by_id(TRAY_ID) {
    ...
    // Entscheid des Eigentuemers 22.09.2026: Die Menueleiste zeigt nur noch
    // das Icon, keinen Text mehr -- weder Meetingtitel noch Countdown
    // ("Meeting • 4m left").
    // Diese Funktion baute den Titel frueher aus dem Terminplan
    // (menu_bar_title) und schrieb ihn per tray.set_title(...). Die
    // Terminplan-Daten selbst bleiben im Aufklapp-Menue erhalten (siehe
    // build_tray_menu/refresh_menu_if_agenda_changed) -- nur der Text neben
    // dem Icon faellt weg. Die Funktion bleibt als No-Op-Wache bestehen,
    // falls ein frueherer Titel je gesetzt war (defensiv, in der Praxis nie
    // der Fall, da MENU_BAR_TITLE nirgends mehr auf Some(..) gesetzt wird).
    fn refresh_menu_bar_title(app: &AppHandle<tauri::Wry>) -> Result<()> {
        ...
        if current_title.is_some() {
            // tray-icon behandelt None als No-Op auf macOS, deshalb explizit
            // ein leerer String, um einen zuvor gesetzten Titel zu loeschen.
            tray.set_title(Some(""))?;
            *current_title = None;
        }
    ...
    fn refresh_icon(app: &AppHandle<tauri::Wry>) -> Result<()> {
                ...
                *task = Some(tauri::async_runtime::spawn(async move {
                    ...
                    loop {
                            ...
                            && let Ok(image) = Image::from_bytes(RECORDING_FRAMES[frame])
                        {
                            ...
                            // set_icon() resets the template flag that the
                            // builder set once at startup (tray-icon crate
                            // hard-codes it to false on every icon swap) --
                            // ohne diese Zeile wird das Icon nach dem ersten
                            // Frame-Wechsel schwarz auf dunkler Leiste.
        ...
        // Gleicher Grund wie bei den Animations-Frames oben: set_icon() setzt
        // das Template-Flag hart auf false zurueck (tray-icon 529+), jeder
        // spaetere Icon-Wechsel braucht die Zeile erneut.
