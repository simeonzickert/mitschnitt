...
impl AppCategory {
    pub fn bundle_ids(&self) -> &'static [&'static str] {
        match self {
            // Die App selbst. Diese Kategorie wird standardmaessig ignoriert,
            // damit die App nicht sich selbst als fremden Mithoerer am
            // Mikrofon meldet. Bis zum 02.09.2026 hiess sie "Anarlog" und
            // fuehrte auch die Kennungen des Originals -- das Original ist
            // aber eine fremde Meeting-App wie Zoom und wird gemeldet
            // (Entscheid, ZICK-270). Schwesterliste: crates/detect.
            Self::Mitschnitt => &[
...
mod tests {
    ...
    // Wache: die eigenen Kennungen muessen in der ignorierten Kategorie
    // liegen, sonst meldet die App sich selbst als fremde App am Mikrofon.
    #[test]
    fn the_fork_is_recognised_as_itself() {
        for id in [
            "media.zickert.mitschnitt",
            "media.zickert.mitschnitt.stable",
            "media.zickert.mitschnitt.staging",
            ...
            assert_eq!(
                AppCategory::find_category(id),
                Some(AppCategory::Mitschnitt),
                "{id} wird nicht als eigene App erkannt"
            );
            assert!(
                !MicNotificationPolicy::default().should_track_app(id),
                "{id} wuerde als fremder Mithoerer gemeldet"
            );
        }
    }

    // Entscheid 02.09.2026 (ZICK-270): das Original ist eine fremde
    // Meeting-App wie Zoom -- keine Kategorie, also gemeldet.
    #[test]
    fn the_original_product_is_a_foreign_meeting_app() {
        for id in [
            "com.hyprnote.dev",
            "com.hyprnote.stable",
            ...
            assert_eq!(
                AppCategory::find_category(id),
                None,
                "{id} steckt noch in einer ignorierten Kategorie"
            );
            assert!(
                MicNotificationPolicy::default().should_track_app(id),
                "{id} wuerde am Mikrofon verschwiegen statt gemeldet"
            );
        }
    }
    ...
    fn test_app_category_find() {
        assert_eq!(
            AppCategory::find_category("media.zickert.mitschnitt"),
            Some(AppCategory::Mitschnitt)
        );
    ...
    fn test_app_category_all_returns_every_variant() {
        ...
        assert!(all.contains(&AppCategory::Mitschnitt));
    ...
    fn test_should_not_track_categorized_app() {
        let policy = MicNotificationPolicy::default();
        assert!(!policy.should_track_app("media.zickert.mitschnitt"));
    ...
    fn test_policy_with_no_ignored_categories_passes_all() {
        ...
        let apps = vec![app("media.zickert.mitschnitt"), app("us.zoom.xos")];
        let ctx = PolicyContext {
            apps: &apps,
            is_dnd: false,
            event_type: MicEventType::Started,
    ...
    fn test_policy_with_selective_ignored_categories() {
        ...
        let result = policy.evaluate(&ctx).unwrap();
        let ids: Vec<_> = result.filtered_apps.iter().map(|a| a.id.as_str()).collect();
        assert_eq!(ids, vec!["media.zickert.mitschnitt", "us.zoom.xos"]);
    }
}
