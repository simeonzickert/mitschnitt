...
/// Platzpruefung.
pub fn resolve_target_vault<R: tauri::Runtime>(
    app: &tauri::AppHandle<R>,
) -> crate::Result<PathBuf> {
    resolve_startup_vault_base(app)
}
...
#[cfg(test)]
pub(crate) async fn legacy_vault_fuer_tests(
    pool: &SqlitePool,
...
pub async fn get_legacy_cleanup_status(
    pool: &SqlitePool,
    eigener_datenordner: Option<&std::path::Path>,
) -> crate::Result<crate::LegacyCleanupStatus> {
    cleanup::get_status(pool, eigener_datenordner).await
}

pub async fn cleanup_legacy_files(
    pool: &SqlitePool,
    eigener_datenordner: Option<&std::path::Path>,
) -> crate::Result<crate::LegacyCleanupResult> {
    cleanup::execute(pool, eigener_datenordner).await
}
...
mod tests {
    ...
    async fn conflict_only_import_preserves_cleanup_guard() {
        ...
        let cleanup_status = cleanup::get_status(db.pool(), Some(std::path::Path::new("/vault")))
            .await
            .unwrap();
    ...
    async fn explicit_retry_normalizes_legacy_conflict_only_run() {
        ...
        let cleanup_status = cleanup::get_status(db.pool(), Some(vault.path()))
            .await
            .unwrap();
        assert!(!cleanup_status.migration_verified);
        assert!(!cleanup_status.available);
    ...
    #[tokio::test]
    async fn verified_file_import_survives_a_reopen_without_rerun() {
