...
/// Import aus dem EIGENEN Datenordner (Start der App). Dieser Lauf darf den
/// Migrationszustand fuehren.
pub async fn import_legacy_vault(
    pool: &SqlitePool,
    vault_base: &Path,
    dry_run: bool,
) -> crate::Result<String> {
    import_vault(pool, vault_base, dry_run, true).await
}

/// Import aus einem FREMDEN Ordner (Installation einer alten App). Gleicher
/// Ablauf, aber der Lauf beansprucht den Migrationszustand nicht -- sonst
/// zeigte der Aufraeum-Knopf hinterher auf den Ordner des Nutzers. Siehe
/// `anlg_db_app::finish_foreign_import_run`.
pub async fn import_foreign_vault(
    pool: &SqlitePool,
    vault_base: &Path,
    dry_run: bool,
) -> crate::Result<String> {
    import_vault(pool, vault_base, dry_run, false).await
}
...
async fn import_vault(
    ...
    if eigener_datenordner {
        ...
        anlg_db_app::begin_foreign_import_run(pool, &run_id, &source_root, dry_run).await?;
    }

    let discovery = match discover_sources(vault_base, !eigener_datenordner) {
    ...
    if eigener_datenordner {
        ...
        anlg_db_app::finish_foreign_import_run(pool, &run_id).await?;
    }
    Ok(run_id)
}
