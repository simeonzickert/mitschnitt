...
pub async fn get_status(
    pool: &SqlitePool,
    eigener_datenordner: Option<&Path>,
) -> crate::Result<crate::LegacyCleanupStatus> {
    Ok(build_plan(pool, eigener_datenordner).await?.status)
}

pub async fn execute(
    pool: &SqlitePool,
    eigener_datenordner: Option<&Path>,
) -> crate::Result<crate::LegacyCleanupResult> {
    let plan = build_plan(pool, eigener_datenordner).await?;
...
async fn build_plan(
    ...
    // Zweiter, unabhaengiger Grund gegen den Datenverlust-Pfad: dieser Knopf
    // loescht ausschliesslich im EIGENEN Datenordner. Selbst wenn ein Lauf
    // gegen eine fremde Quelle den Zeiger `latest_run_id` doch einmal an sich
    // reissen sollte, zeigt sein `source_root` woanders hin -- und dann wird
    // hier nichts angeboten. Ist der eigene Ordner nicht bekannt, wird
    // ebenfalls blockiert: nicht wissen ist kein Freibrief.
    let Some(eigener_datenordner) = eigener_datenordner else {
        return Ok(blocked_plan(
            "Legacy cleanup cannot verify its own data directory",
            state.latest_run_id,
            migration_ready,
        ));
    };
    if !gehoert_zum_eigenen_datenordner(&source_root, eigener_datenordner) {
        return Ok(blocked_plan(
            "Legacy migration source is outside this installation's data directory",
            state.latest_run_id,
            migration_ready,
        ));
    }
...
/// Vergleicht aufgeloest, damit ein Symlink oder ein `..` im Pfad nicht an der
/// Pruefung vorbeikommt. Laesst sich eine Seite nicht aufloesen, gilt sie als
/// nicht zugehoerig.
fn gehoert_zum_eigenen_datenordner(source_root: &Path, eigener_datenordner: &Path) -> bool {
    let (Ok(quelle), Ok(eigen)) = (
        source_root.canonicalize(),
        eigener_datenordner.canonicalize(),
    ) else {
        return false;
    };
    quelle == eigen || quelle.starts_with(&eigen)
}
...
mod tests {
    ...
    /// Das Netz unter der Ursache, unabhaengig von ihr geprueft: hier ist der
    /// Zeiger AUSDRUECKLICH auf einen Lauf gegen einen fremden Ordner gesetzt
    /// und der Zustand als geprueft markiert -- also genau die Lage, die nach
    /// dem Fund vom 10.09.2026 nicht mehr eintreten kann. Selbst dann darf
    /// hier nichts zum Loeschen angeboten werden.
    #[tokio::test]
    async fn fremder_quellordner_wird_niemals_zum_loeschen_angeboten() {
        let db = anlg_db_core::Db::connect_memory_plain().await.unwrap();
        anlg_db_app::prepare_schema(&db).await.unwrap();

        let fremd = tempfile::tempdir().unwrap();
        let eigener_datenordner = tempfile::tempdir().unwrap();
        let notiz = b"# Fremde Notiz";
        std::fs::write(fremd.path().join("settings.json"), notiz).unwrap();

        seed_verified_import(
            db.pool(),
            fremd.path(),
            &[("settings.json", &sha256(notiz))],
        )
        .await;

        let status = get_status(db.pool(), Some(eigener_datenordner.path()))
            .await
            .unwrap();
        assert!(
            !status.available,
            "fremder Ordner wurde zum Loeschen angeboten"
        );
        assert_eq!(status.file_count, 0);
        assert!(status.blocking_reason.is_some());

        // Und der Vollzug verweigert ebenfalls, statt sich auf die Anzeige zu
        // verlassen: die Datei muss danach noch dastehen.
        let _ = execute(db.pool(), Some(eigener_datenordner.path())).await;
        assert!(
            fremd.path().join("settings.json").exists(),
            "die fremde Datei wurde geloescht"
        );
    }

    /// Ohne Kenntnis des eigenen Datenordners wird ebenfalls blockiert.
    #[tokio::test]
    async fn unbekannter_eigener_datenordner_blockiert() {
        let db = anlg_db_core::Db::connect_memory_plain().await.unwrap();
        anlg_db_app::prepare_schema(&db).await.unwrap();
        let vault = tempfile::tempdir().unwrap();
        let notiz = b"# Notiz";
        std::fs::write(vault.path().join("settings.json"), notiz).unwrap();
        seed_verified_import(
            db.pool(),
            vault.path(),
            &[("settings.json", &sha256(notiz))],
        ...
        let status = get_status(db.pool(), None).await.unwrap();
    ...
    async fn removes_only_verified_json_and_markdown_sources() {
        ...
        let status = get_status(db.pool(), Some(vault.path())).await.unwrap();
        assert!(status.migration_ready);
        assert!(status.available);
        assert_eq!(status.file_count, 2);
        assert_eq!(status.total_bytes, (settings.len() + note.len()) as u64);

        let result = execute(db.pool(), Some(vault.path())).await.unwrap();
        ...
        assert!(
            get_status(db.pool(), Some(vault.path()))
                .await
                .unwrap()
                .already_cleaned
        );
    ...
    async fn changed_source_blocks_all_cleanup() {
        ...
        assert!(execute(db.pool(), Some(vault.path())).await.is_err());
        assert!(vault.path().join("settings.json").exists());
