...
fn collect_files(
    ...
    for entry in entries {
        let path = entry.path();
        if path.is_dir() {
            collect_files(vault_base, &path, files, summary_pairs, fremde_quelle)?;
            continue;
        }

        let relative_path = normalized_relative_path(vault_base, &path);
        if let Some(summary_pair) =
            detect_summary_pair(vault_base, &path, &relative_path, fremde_quelle)
        {
