...
pub(super) fn detect_summary_pair(
    ...
    if hidden_document.session_id != canonical_document.session_id {
        return None;
    }
    // Beim Import aus dem EIGENEN Datenordner bleibt es bei der urspruenglichen
    // Bedingung: dort werden die Quelldateien spaeter aufgeraeumt, und eine
    // abweichende versteckte Fassung ist die letzte Kopie ihres Textes.
    if !fremde_quelle && hidden_document.id != canonical_document.id {
        return None;
    }
    ...
    let gleicher_stand = (hidden_empty
    ...
    // Bei einem FREMDEN Bestand genuegt es, dass die kanonische Fassung Inhalt
    // hat. `.md` ist dort der Altstand von `_summary.md` -- gemessen an
    // der gemessenen Installation (10.09.2026): 146 Ordner tragen beide Dateien,
    // `_summary.md` ist in 146 von 146 juenger und in keinem einzigen inhalts-
    // gleich. Als zweites Dokument gefuehrt ergaeben sie 146 Dubletten.
    //
    // Der Text geht dabei nicht verloren: die fremde Quelle wird nur gelesen,
    // die Datei bleibt liegen. Genau darauf stuetzt sich der Unterschied zum
    // eigenen Datenordner, wo derselbe Fall eine Rettungskopie rechtfertigt.
    let hide_hidden_source = if fremde_quelle {
        hidden_empty || !canonical_empty
    } else {
        gleicher_stand
    };
