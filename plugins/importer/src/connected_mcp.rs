...
pub async fn begin_connection(
    ...
    let client = manager
        .register_client("Mitschnitt", &redirect_uri, &scope_refs)
        .await
        .map_err(|error| auth_error(provider, error))?;
...
async fn receive_authorization_callback(
    ...
    let (status, title, message) = match &result {
        Ok(_) => (
            ...
            "Your meeting history is being brought into Mitschnitt. You can close this window.",
        ),
        Err(_) => (
            "400 Bad Request",
            format!("{provider_name} connection failed"),
            "Return to Mitschnitt and try connecting again.",
        ),
    };
    let body = format!(
