...
mod tests {
    ...
    // Mitschnitt-Fork (ISA N8, ZICK-251): a deleted meeting takes its
    // pictures with it. The note attachments live under
    // <session>/attachments/<id>; remove_dir_all is recursive, and this
    // pins that the folder does not survive as a stray sibling.
    #[test]
    fn delete_session_dir_takes_the_note_attachments_with_it() {
        let temp = TempDir::new().unwrap();
        let session_dir = temp.path().join(UUID_1);
        let attachments = session_dir.join("attachments");
        std::fs::create_dir_all(&attachments).unwrap();
        std::fs::write(attachments.join("diagram.png"), b"png").unwrap();

        delete_session_dir(&session_dir).unwrap();

        assert!(!attachments.join("diagram.png").exists());
