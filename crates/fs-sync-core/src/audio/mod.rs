...
mod tests {
    ...
    fn import_into_temp(source: &str) -> (TempDir, std::path::PathBuf) {
        let temp = TempDir::new().unwrap();
        let tmp_path = temp.path().join("tmp.mp3");
        let target_path = temp.path().join("target.mp3");

        let result = import_audio(std::path::Path::new(source), &tmp_path, &target_path);
    ...
    macro_rules! test_import_audio {
        ($($name:ident: $path:expr),* $(,)?) => {
            $(
                #[test]
                fn $name() {
                    let (_temp, _target) = import_into_temp($path);
                }
            )*
        };
    }
    ...
    fn test_import_stereo_mp3() {
        let (_temp, target_path) = import_into_temp(anlg_fixtures::SPEECH_STEREO_MP3);
