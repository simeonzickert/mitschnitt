...
/// Human ids of everyone in the meeting except the current user.
///
/// `self_emails` carries every address that belongs to the current user, not
/// just the one on their own contact record: that record can have no email at
/// all, and one address only ever covers one calendar. The calendar's own
/// `is_current_user` flag is the reliable source; the caller resolves it.
pub fn remote_participant_human_ids<'a>(
    participants: impl IntoIterator<Item = (&'a str, &'a str)>,
    owner_user_id: &str,
    self_emails: impl IntoIterator<Item = &'a str>,
) -> Vec<String> {
    let self_emails: Vec<&str> = self_emails
        .into_iter()
...
mod tests {
    ...
    #[test]
    fn remote_participants_drop_every_address_of_the_current_user() {
        // The owner contact has no email, so the only thing separating the
        // calendar copies from the remotes is the calendar's own flag -- and
        // the same person appears under two work addresses.
        assert_eq!(
            remote_participant_human_ids(
                [
                    ("john", ""),
                    ("john-work", "John@work.example"),
                    ("john-side", "john@side.example"),
                    ("marco", "marco@example.com"),
                ],
                "john",
                ["john@work.example", "john@side.example"],
            ),
            vec!["marco".to_string()]
        );
    }
    ...
    #[test]
    fn remote_participants_keep_everyone_when_no_self_address_is_known() {
        assert_eq!(
            remote_participant_human_ids(
                [("john-cal", "john@example.com"), ("marco", "marco@x.test")],
                "john",
                [""],
            ),
            vec!["john-cal".to_string(), "marco".to_string()]
        );
    }
