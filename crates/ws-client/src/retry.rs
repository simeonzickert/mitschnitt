...
impl fmt::Debug for RedactedRequest<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RedactedRequest")
            .field("uri", &RedactedUri(self.0.uri()))
            .field("headers", &RedactedHeaders(self.0.headers()))
            .finish()
    }
}

/// The URI with its secrets replaced (D1, Fix-Runde 1d; G3, Fix-Runde 2).
/// Until 02.09.2026 the `Debug` of the request printed the URI verbatim, on
/// INFO, at every live connection -- and the Google live adapter hangs the
/// API key on the URL as `?key=` (owhisper-client, google_generative_ai/
/// live.rs), so the log file carried the key each time a meeting started.
///
/// A query parameter is redacted by its NAME, on every host: secret-named
/// ones (`key`, `api_key`, `token`, `jwt`, … -- `is_secret_param_name`, the
/// same list as plugins/tracing and apps/desktop/src/error-reporting.ts) go,
/// diagnostic ones (`model`, `language`) stay, so the line still says what
/// was asked for. Until Fix-Runde 2 a loopback host kept its whole query and
/// every other host lost every value. Userinfo (`user:pass@`) goes whole; a
/// bracketed IPv6 host is an authority like any other.
struct RedactedUri<'a>(&'a tokio_tungstenite::tungstenite::http::Uri);

impl fmt::Debug for RedactedUri<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let uri = self.0;
        ...
        if let Some(authority) = uri.authority() {
            ...
            match authority.rfind('@') {
                Some(at) => {
                    ...
                    rendered.push_str(&authority[at + 1..]);
                }
                None => rendered.push_str(authority),
            }
        }
        rendered.push_str(uri.path());
        if let Some(query) = uri.query() {
            rendered.push('?');
            rendered.push_str(&redact_query_values(query));
        }
...
mod tests {
    ...
    use super::{RedactedHeaders, RedactedUri, redact_header_value};
    ...
    // D1 (Fix-Runde 1d): the Google live adapter puts the key on the URL.
    #[test]
    fn redacted_uri_hides_query_values_but_keeps_their_names() {
        let uri: tokio_tungstenite::tungstenite::http::Uri =
            "wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent?key=AIzaSyD-1234567890abcdefghijklmnop&model=x"
                .parse()
                .unwrap();

        let redacted = format!("{:?}", RedactedUri(&uri));

        assert!(!redacted.contains("AIzaSyD"), "{redacted}");
        assert!(
            redacted.contains(
                "wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent?key=<redacted>&model=x"
            ),
            "{redacted}"
        );
    }

    // G3 (Fix-Runde 2): a parameter falls by its name, on every host; the
    // diagnostic ones stay readable on every host. Until this round a
    // loopback host kept a `key=` whole and every other host lost `model=`.
    #[test]
    fn redacted_uri_keeps_a_loopback_query_readable_except_its_secrets() {
        let uri: tokio_tungstenite::tungstenite::http::Uri =
            "ws://127.0.0.1:50060/v1/listen?key=abc&model=x"
                .parse()
                .unwrap();
        ...
        assert!(
            redacted.contains("ws://127.0.0.1:50060/v1/listen?key=<redacted>&model=x"),
            "{redacted}"
        );
    }

    #[test]
    fn redacted_uri_handles_a_bracketed_ipv6_host() {
        let uri: tokio_tungstenite::tungstenite::http::Uri =
            "wss://[2001:db8::1]:443/ws?key=abc&language=de"
                .parse()
                .unwrap();
        ...
        assert!(
            redacted.contains("wss://[2001:db8::1]:443/ws?key=<redacted>&language=de"),
            "{redacted}"
        );
    }

    #[test]
    fn redacted_uri_hides_userinfo() {
        let uri: tokio_tungstenite::tungstenite::http::Uri =
            "wss://user:pass@api.example.com/ws?token=abc"
                .parse()
                .unwrap();
        ...
        assert!(
            redacted.contains("wss://<redacted>@api.example.com/ws?token=<redacted>"),
            "{redacted}"
        );
    }

    #[test]
    fn redacted_uri_without_query_is_verbatim() {
        let uri: tokio_tungstenite::tungstenite::http::Uri =
            "wss://api.deepgram.com/v1/listen".parse().unwrap();

        assert_eq!(
            format!("{:?}", RedactedUri(&uri)),
