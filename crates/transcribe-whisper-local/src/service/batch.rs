...
use super::{TARGET_SAMPLE_RATE, build_batch_model, build_metadata, transcribe_chunk};
    ...
    S: Source<Item = f32>,
{
    ...
    let mut model = build_batch_model(loaded_model, params)?;
    ...
    if let Some(obj) = metadata_json.as_object_mut() {
        ...
        // Fork-Abweichung (31.08.2026): Whisper liefert SEGMENT-Zeiten, die
        // Wortzeiten darin sind gleichverteilt (`build_stream_words`). Das
        // Etikett sagt genau das -- und es entscheidet im Frontend darueber, ob
        // man ins Transkript springen kann (`stt/timing.ts`). Ohne das Feld
        // faellt die Oberflaeche auf "provider_word" zurueck und behauptete
        // damit eine Genauigkeit, die hier nicht vorliegt.
        //
        // NICHT auf "provider_word" heben, solange die Wortzeiten von
        // whisper.cpp nicht belegt sind: am 31.08.2026 an echtem Material
        // gemessen spannte dort ein einzelnes Wort 16 Sekunden.
        obj.insert(
            "timing_source".to_string(),
            serde_json::json!("provider_segment_interpolated"),
        );
    }
...
mod tests {
    ...
    /// Handprobe gegen echtes Material -- laeuft nicht im normalen Testlauf,
    /// weil sie ein ggml-Modell und eine Aufnahme auf der Platte braucht.
    ///
    ///   MITSCHNITT_WHISPER_MODEL=/pfad/ggml-large-v3-turbo.bin \
    ///   MITSCHNITT_WHISPER_MIC=/pfad/mic.wav \
    ///   MITSCHNITT_WHISPER_SPK=/pfad/spk.wav \
    ///   cargo test -p transcribe-whisper-local batch_probe -- --ignored --nocapture
    ///
    /// Die beiden Mono-Spuren werden hier zu EINER Stereodatei zusammengesetzt,
    /// weil genau das der Fall ist, den die App faehrt: eine Aufnahme, zwei
    /// Kanaele, getrennt transkribiert. Wer nur eine Monospur durchschickt,
    /// prueft die Kanaltrennung nicht.
    #[test]
    #[ignore = "braucht ein ggml-Modell und echte Aufnahmen auf der Platte"]
    fn batch_probe_against_real_audio() {
        use std::path::Path;

        let model_path =
            std::env::var("MITSCHNITT_WHISPER_MODEL").expect("MITSCHNITT_WHISPER_MODEL fehlt");
        let mic = std::env::var("MITSCHNITT_WHISPER_MIC").expect("MITSCHNITT_WHISPER_MIC fehlt");
        let spk = std::env::var("MITSCHNITT_WHISPER_SPK").expect("MITSCHNITT_WHISPER_SPK fehlt");

        let stereo = interleave_to_stereo(Path::new(&mic), Path::new(&spk));
        let loaded = crate::service::load_model(Path::new(&model_path)).expect("Modell laedt");
        let params = crate::service::parse_listen_params("language=de").expect("Parameter");

        let started = std::time::Instant::now();
        let response = super::transcribe_batch(
            stereo.path(),
            &params,
            &loaded,
            Path::new(&model_path),
        ...
        .expect("Batch laeuft durch");
        let elapsed = started.elapsed();

        println!("\n=== metadata ===\n{}", response.metadata);
        println!("Dauer des Laufs: {:.1}s", elapsed.as_secs_f64());

        assert_eq!(
            response.metadata["timing_source"], "provider_segment_interpolated",
            "das Etikett fehlt oder ist falsch"
        );
        assert_eq!(
            response.results.channels.len(),
            2,
            "beide Spuren muessen getrennt transkribiert werden"
        );

        for (index, channel) in response.results.channels.iter().enumerate() {
            ...
            for word in alternative.words.iter().take(12) {
                println!("  {:>8.2}s - {:>8.2}s  {}", word.start, word.end, word.word);
            }
            ...
            // Der eigentliche Befund: die Zeiten duerfen NICHT auf einem festen
            // Raster liegen und das erste Wort nicht bei 0, wenn die Spur mit
            // Stille beginnt.
            assert!(
                alternative.words.iter().all(|word| word.end >= word.start),
                "Kanal {index} hat rueckwaerts laufende Wortzeiten"
            );
        }
    }

    fn interleave_to_stereo(
        left: &std::path::Path,
        right: &std::path::Path,
    ) -> tempfile::NamedTempFile {
        fn read(path: &std::path::Path) -> (hound::WavSpec, Vec<f32>) {
            let mut reader = hound::WavReader::open(path).expect("wav liest");
            ...
            let samples: Vec<f32> = match spec.sample_format {
                ...
                hound::SampleFormat::Int => reader
                    .samples::<i32>()
                    .map(|s| {
                        s.unwrap() as f32 / i32::pow(2, spec.bits_per_sample as u32 - 1) as f32
                    })
                    .collect(),
            };
            (spec, samples)
        ...
        let (spec, left_samples) = read(left);
        let (_, right_samples) = read(right);
        ...
        .unwrap();
        for index in 0..left_samples.len().min(right_samples.len()) {
            writer.write_sample(left_samples[index]).unwrap();
            writer.write_sample(right_samples[index]).unwrap();
        }
