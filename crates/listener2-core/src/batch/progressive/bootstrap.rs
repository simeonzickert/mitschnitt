use owhisper_client::{LocalServerAdapter, OpenAIAdapter, WhisperCppAdapter};
...
> {
    match args.progressive_provider {
        ProgressiveProvider::LocalServer => {
            spawn_local_server_progressive_batch_task(args, myself).await
        }
...
async fn spawn_local_server_progressive_batch_task(
    args: BatchArgs,
    myself: ActorRef<BatchMsg>,
) -> Result<
    (
...
> {
    ...
    let span = tracing::info_span!(
        "local_server_progressive_batch",
    ...
    let rx_task = tokio::spawn(
        async move {
            tracing::info!("local server progressive batch task: starting");

            let stream = match LocalServerAdapter::transcribe_file_streaming(
                &args.base_url,
                &args.api_key,
                &args.listen_params,
                &args.file_path,
            ...
            .await
            {
                ...
                Err(err) => {
                    report_stream_start_failure(
                        ...
                        &args.provider_label,
                        &err,
                        "local server progressive batch task failed to start",
                    );
                    return;
                }
            };
            ...
            process_provider_stream(
                ...
                shutdown_rx,
                &args.provider_label,
                "local server progressive batch",
            )
            .await;
            tracing::info!("local server progressive batch task exited");
        }
        .instrument(span),
    );
