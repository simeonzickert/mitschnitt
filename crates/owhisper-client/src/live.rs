...
use crate::{DeepgramAdapter, RealtimeSttAdapter, normalize_listen_params};
...
impl<A: RealtimeSttAdapter> ListenClientBuilder<A> {
    ...
    async fn build_request(
        ...
        // C5 (Opus 7, Review 02.09.2026): hier wurde jeder URL `?provider=`
        // angehaengt und einer URL der Gegenstelle des Originals ein
        // `Authorization: Bearer` samt Geraete-Fingerabdruck mitgegeben -- das
        // Proxy-Protokoll von api.anarlog.so. Dieser Fork baut keinen Client
        // gegen das Original; jede Adresse bekommt den Auth-Header ihres
        // Adapters, sonst nichts.
