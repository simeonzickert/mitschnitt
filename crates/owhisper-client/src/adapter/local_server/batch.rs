...
impl BatchSttAdapter for LocalServerAdapter {
    ...
    fn is_supported_languages(
        ...
        LocalServerAdapter::is_supported_languages_batch(languages, model)
    }
...
fn linear16_file_stream(
    ...
    futures_util::stream::unfold(file, move |mut file| async move {
        ...
        match file.read(&mut bytes).await {
            ...
            Err(error) => {
                tracing::warn!(error.message = %error, "local_server_linear16_spool_read_failed");
                None
            }
        }
    })
