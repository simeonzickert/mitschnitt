...
fn build_batch_url(api_base: &str, params: &ListenParams) -> url::Url {
    ...
    // Getrennt von `keywords` uebertragen: der Dienst baut daraus die
    // Kontext-Vorgabe fuer whisper.cpp, und dort duerfen NUR richtige
    // Schreibweisen landen.
    for term in &params.vocabulary {
        url.query_pairs_mut().append_pair("vocabulary", term);
    }
