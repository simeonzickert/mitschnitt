...
pub fn documented_language_codes_live() -> Vec<String> {
    ...
    codes.extend(
        anlg_language::PARAKEET_TDT_V3_LANGUAGE_CODES
            .iter()
            .copied(),
    );
...
// NICHT UMBENENNEN. Fremder Hostname, den wir nicht besitzen. Er wird hier nur
// WIEDERERKANNT, um Anfragen an genau diese Gegenstelle zu erkennen; ein
// erfundener eigener Name laesst die Erkennung still ins Leere laufen.
...
fn is_local_server(base_url: &str) -> bool {
...
pub enum AdapterKind {
    ...
    Cartesia,
    // Hiess bis zum 01.09.2026 `Argmax`, und der Name war falsch. Dieser
    // Zweig ist der Weg zu JEDEM lokalen Deepgram-kompatiblen Server auf
    // dieser Maschine -- `is_local_server` unten trifft auf jede
    // Loopback-Adresse ausser dem anarlog-Proxy. Praktisch faehrt genau ein
    // Server darauf: der interne whisper.cpp-Dienst aus
    // `plugins/local-stt/src/server/internal.rs`. Der Argmax-Sidecar, der
    // frueher ebenfalls hier landete, ist ersatzlos entfernt.
    //
    // Der Name ist deshalb keine Kosmetik: wer `Argmax` liest, haelt diesen
    // Zweig fuer tot und nimmt ihn beim naechsten Aufraeumen mit -- und
    // schaltet damit die lokale Whisper-Transkription ab.
    #[strum(serialize = "local-server")]
    LocalServer,
    ...
    #[strum(serialize = "xai")]
    Xai,
    // NICHT UMBENENNEN. Diese Kennung wird so in den Einstellungen und in der
    // Datenbank abgelegt (`current_stt_provider`) und an rund vierzig Stellen
    // im Frontend verglichen. Ein neuer Wert braucht eine Migration ueber den
    // Bestand und bringt nichts Sichtbares: der Anzeigename lautet bereits
    // "Mitschnitt", nur der gespeicherte Schluessel traegt noch den Altnamen.
    #[strum(serialize = "anarlog")]
    Anarlog,
}

impl AdapterKind {
    /// Kennungen, fuer die dieser Fork keinen Netz-Client mehr baut, mit dem
    /// Grund als lesbare Meldung. `Anarlog` bleibt als persistierte Kennung
    /// (Datenbank, Draht, `#[serde(alias = "hyprnote")]` in listener2-core)
    /// lesbar; nur der Client dahinter ist weg (Grok-Review 02.09.2026, F4).
    pub fn network_client_unavailable(&self) -> Option<&'static str> {
        match self {
            Self::Anarlog => Some(
                "provider \"anarlog\" removed in this fork: it addressed api.anarlog.so, the original's backend, and no network client is built for it any more",
            ),
            _ => None,
        }
    }
    ...
    pub fn language_support_live(
        ...
        match self {
            ...
            Self::LocalServer => LocalServerAdapter::language_support_live(languages, model),
    ...
    pub fn language_support_batch(
        ...
        match self {
            ...
            Self::LocalServer => LocalServerAdapter::language_support_batch(languages, model),
