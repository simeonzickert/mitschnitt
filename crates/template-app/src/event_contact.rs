...
mod tests {
    ...
    Use `null` for `email` when the event identifies the person but no candidate email matches. Use `null` for `companyName` when no company is clear. Infer obvious company names from non-personal email domains, for example `tom@kestroll.example` means `Kestroll`. Use `{"contacts":[]}` when no contact should be extracted.
    "#);
    ...
    tpl_snapshot!(
        ...
        EventContactUser {
            ...
            candidates: vec![
                ...
                EventContactCandidate {
                    name: Some("daniel.lee@example.com".to_string()),
                    email: Some("daniel.lee@example.com".to_string()),
    ...
    - daniel.lee@example.com <daniel.lee@example.com>
