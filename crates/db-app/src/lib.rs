...
// "update Mitschnitt" dialog instead of misbehaving on it.
/// Der Vorlagen-Step, an EINER Stelle (D6, Fix-Runde 1d): die Kennung
/// steht im Migrationsplan, in zwei Reparaturen (migration_sql) und -- als
/// Versionsnummer -- in der Abfrage der Migrations-Historie. Vorher standen
/// drei Schreibweisen im Code, und eine haette abweichen koennen, ohne dass
/// ein Test es merkt.
pub const TEMPLATES_MIGRATION_ID: &str = "20260413020000_templates";
const TEMPLATES_MIGRATION_VERSION: i64 = 20260413020000;

pub const APP_MIGRATION_STEPS: &[anlg_db_migrate::MigrationStep] = &[
    anlg_db_migrate::MigrationStep {
        id: TEMPLATES_MIGRATION_ID,
    ...
    anlg_db_migrate::MigrationStep {
        id: "20260901120000_drop_cloud_layer",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260901120000_drop_cloud_layer.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260902001000_mitschnitt_standard_vorlage",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260902001000_mitschnitt_standard_vorlage.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260902001100_mitschnitt_standard_als_standard",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260902001100_mitschnitt_standard_als_standard.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260902001200_willkommen_titel_mitschnitt",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260902001200_willkommen_titel_mitschnitt.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904120000_adaptive_minutes_vorlage",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904120000_adaptive_minutes_vorlage.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904140000_mitschnitt_kompakt_vorlage",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904140000_mitschnitt_kompakt_vorlage.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904140100_mitschnitt_kompakt_als_standard",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904140100_mitschnitt_kompakt_als_standard.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904150000_vorlagen_aufraeumen",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904150000_vorlagen_aufraeumen.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904150100_wahl_nach_aufraeumen",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904150100_wahl_nach_aufraeumen.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260904160000_disziplin_in_den_rahmen",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260904160000_disziplin_in_den_rahmen.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260907120000_bitte_gegenpruefen_in_den_rahmen",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260907120000_bitte_gegenpruefen_in_den_rahmen.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260909120000_voiceprint_exemplars_isolated_mic",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260909120000_voiceprint_exemplars_isolated_mic.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260910120000_import_run_audio_counters",
        scope: anlg_db_migrate::MigrationScope::Plain,
        sql: include_str!("../migrations/20260910120000_import_run_audio_counters.sql"),
    },
    anlg_db_migrate::MigrationStep {
        id: "20260910130000_import_run_session_counters",
        scope: anlg_db_migrate::MigrationScope::Plain,
...
pub enum AppSchemaError {
    ...
    /// Ein Reparaturpfad wollte einen Step nachspielen, der nicht in
    /// APP_MIGRATION_STEPS registriert ist (C8 (6), Review 02.09.2026).
    MissingMigrationStep(&'static str),
}
...
impl std::fmt::Display for AppSchemaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingMigrationStep(id) => {
                write!(f, "Migrations-Step {id} ist nicht registriert")
            }
        }
    }
}
...
/// Der einzige Einstieg, der eine Datenbank vollstaendig auf den aktuellen
/// Stand bringt. `anlg_db_migrate::migrate` allein faehrt nur die Steps: nach
/// einem Verlust der templates-Tabelle steht sie danach zwar wieder, aber
/// ohne die 17 Upstream-Vorlagen, weil deren Seed-Step schon verbucht ist.
/// prepare_schema haengt die Reparaturen davor und dahinter (verlorene
/// Tabelle vor migrate anlegen, beide Vorlagen-Seeds nachspielen, icon_json
/// nachziehen). Wer migrate direkt aufruft, umgeht diesen Vertrag -- der
/// Test migrate_alone_leaves_a_lost_templates_table_without_the_upstream_seeds
/// haelt den Unterschied fest (C8 (7), Review 02.09.2026).
...
pub async fn prepare_schema_with_progress(
    ...
    if templates_missing_before_migration && templates_migration_recorded(db.pool()).await? {
        // Mitschnitt-Fork (F6): eine VERLORENE Tabelle muss VOR migrate wieder
        // stehen. Steht der Vorlagen-Seed (20260902001000) noch aus, starb
        // migrate sonst an "no such table: templates", und die Reparatur
        // dahinter kam nie dran. Verloren heisst: TEMPLATES_MIGRATION_ID ist
        // in _sqlx_migrations verbucht (templates_migration_recorded), die
        // Tabelle fehlt trotzdem -- dann wird der Step hier ausserhalb der
        // Historie nachgespielt, ueber migration_sql, also mit demselben SQL
        // wie im Migrationsplan. Eine frische Datenbank (Step nicht verbucht)
        // geht ganz durch migrate -- bis C8 (5) feuerte dieser Zweig auf jeder
        // frischen Datenbank und trug nur, weil der Step `IF NOT EXISTS` sagt.
        // Ohne icon_json: den Icon-Step (20260712170000) spielt migrate nach,
        // wenn er aussteht; ist er verbucht, haengt repair_missing_core_tables
        // die Spalte ueber den has_icon_json-Check an.
        sqlx::query(migration_sql(TEMPLATES_MIGRATION_ID)?)
            .execute(db.pool())
            .await?;
    }
    ...
    workspace::ensure_workspace_binding(db.pool()).await?;
    Ok(())
}
...
// Builds of 1.4.10 without the transactional alter runner executed
...
/// Ob der Vorlagen-Step (TEMPLATES_MIGRATION_ID) in der Migrations-Historie
/// als erfolgreich verbucht ist. Ohne _sqlx_migrations-Tabelle (frische
/// Datenbank) ist die Antwort nein.
async fn templates_migration_recorded(pool: &sqlx::SqlitePool) -> Result<bool, sqlx::Error> {
    ...
    .bind(TEMPLATES_MIGRATION_VERSION)
    .fetch_one(pool)
    .await
}

/// Das SQL eines registrierten Steps -- fuer Reparaturen, die einen Step
/// ausserhalb der Migrations-Historie nachspielen. Ein Lookup statt eines
/// zweiten include_str!, damit Reparatur und Step nie auseinanderlaufen. Ein
/// unbekannter Step ist ein Fehler, kein Panic: der Aufrufer sitzt im
/// Startpfad (C8 (6), Review 02.09.2026).
fn migration_sql(id: &'static str) -> Result<&'static str, AppSchemaError> {
    APP_MIGRATION_STEPS
        .iter()
        .find(|step| step.id == id)
        .map(|step| step.sql)
        .ok_or(AppSchemaError::MissingMigrationStep(id))
}
...
///
/// Alle Seeds sind INSERT OR IGNORE -- was der Nutzer bearbeitet hat, bleibt;
/// was fehlt, kommt zurueck. Der Aufraeum-Step loescht nur, was Feld fuer Feld
/// dem Auslieferungsstand entspricht, also nie eine bearbeitete Zeile.
///
/// Nie die Einstellungs-Steps (20260902001100, 20260904140100,
/// 20260904150100): die tragen Wahl und Sprache, und eine Reparatur darf
/// keine Wahl ueberschreiben.
async fn replay_template_seeds(pool: &sqlx::SqlitePool) -> Result<(), AppSchemaError> {
    for id in [
        "20260524000000_default_templates",
        "20260902001000_mitschnitt_standard_vorlage",
        "20260904120000_adaptive_minutes_vorlage",
        "20260904140000_mitschnitt_kompakt_vorlage",
        "20260904150000_vorlagen_aufraeumen",
        // Zuletzt der Disziplin-Step, sonst braechte eine Reparatur genau die
        // Fassungen zurueck, die er ersetzt hat: die Seeds sind INSERT OR
        // IGNORE und greifen deshalb NUR nach einer Reparatur -- also im
        // einzigen Fall, in dem die Zeile fehlt. Ohne diesen Eintrag stuenden
        // danach wieder englische Ueberschriften und die ungetrimmte
        // Beschreibung von 'mitschnitt-kompakt' in der Liste.
        "20260904160000_disziplin_in_den_rahmen",
        // Und dahinter der Gegenpruef-Step aus demselben Grund: er STREICHT
        // einen Abschnitt aus allen sieben Vorlagen. Fehlt er hier, stuende
        // nach jeder Tabellen-Reparatur wieder "Zahlen und Namen zur
...
async fn repair_missing_core_tables(
    ...
    .await?;
    if !has_icon_json {
        sqlx::query(migration_sql("20260712170000_template_icons")?)
            .execute(pool)
            .await?;
    }

    if templates_missing_before_migration {
        replay_template_seeds(pool).await?;
    }
