...
pub async fn begin_legacy_import_run(
    ...
    begin_run(pool, run_id, source_root, dry_run, true).await
}

/// Beginn eines Laufs gegen eine FREMDE Quelle. Wie `begin_legacy_import_run`,
/// aber ohne `storage_migration_state` anzufassen.
///
/// Der Zeiger wird an ZWEI Stellen geschrieben, hier und in
/// `finish_legacy_import_run`. Beide muessen fuer Fremdlaeufe schweigen --
/// eine allein reicht nicht, sonst zeigt der Aufraeum-Knopf nach dem Start des
/// Laufs auf den fremden Ordner, auch wenn der Abschluss es nicht mehr tut.
pub async fn begin_foreign_import_run(
    pool: &SqlitePool,
    run_id: &str,
    source_root: &str,
    dry_run: bool,
) -> Result<(), sqlx::Error> {
    begin_run(pool, run_id, source_root, dry_run, false).await
}
...
async fn begin_run(
    ...
    if !dry_run && eigener_datenordner {
        sqlx::query(
...
pub async fn finish_legacy_import_run(
    ...
    finish_run(pool, run_id, true).await
}

/// Abschluss eines Laufs gegen eine FREMDE Quelle (Import aus der Installation
/// einer alten App). Er schreibt sein Ergebnis wie jeder andere Lauf, fasst
/// aber `storage_migration_state` NICHT an.
///
/// Der Grund ist ein Datenverlust-Pfad, kein Schoenheitsfehler: der Zeiger
/// `latest_run_id` ist genau das, woran der Aufraeum-Knopf ablaesst, WELCHEN
/// Ordner er leerraeumen darf (`import/cleanup.rs`). Duerfte ein Fremdlauf
/// diesen Zeiger setzen, richtete sich der Knopf gegen den Datenordner der
/// alten App -- an dem gemessenen Bestand waeren das 1.124 `.json`- und `.md`-Dateien
/// plus die dann leeren Sitzungsordner gewesen, waehrend die 146 Tondateien
/// ohne jede Zuordnung liegen blieben. Wer kein zweites Exemplar hat, verliert
/// damit sein Archiv.
pub async fn finish_foreign_import_run(
    pool: &SqlitePool,
    run_id: &str,
) -> Result<String, sqlx::Error> {
    finish_run(pool, run_id, false).await
}

async fn finish_run(
    pool: &SqlitePool,
    run_id: &str,
    besitzt_migrationszustand: bool,
) -> Result<String, sqlx::Error> {
