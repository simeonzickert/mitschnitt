...
async fn first_import_restores_legacy_edits_over_untouched_default_templates() {
    let db = test_db().await;
    // Die unberuehrte Standard-Vorlage muss hier wirklich liegen, sonst
    // pruefte dieser Test nur einen Import in eine Luecke. Sie kommt seit dem
    // Aufraeumen vom 04.09.2026 (Step 20260904150000) nicht mehr von selbst
    // aus dem Seed -- 'default-daily-standup' gehoert zu den dreizehn
    // entfernten. Der Import darf sie trotzdem ueberschreiben, und genau das
    // ist die Zusage dieses Tests.
    sqlx::query(
        "INSERT INTO templates (id, title, description, pinned, pin_order, category, targets_json, sections_json)
         VALUES ('default-daily-standup', 'Daily Standup', 'For quick daily syncs to share progress and blockers',
                 0, NULL, 'Engineering', '[\"Software Engineer\"]', '[]')",
    )
    .execute(db.pool())
    .await
    .unwrap();
