...
impl DeviceChangeWatcher {
    ...
    fn event_loop(event_rx: Receiver<DeviceSwitch>, actor: ActorRef<SourceMsg>) {
        loop {
            match event_rx.recv() {
                Ok(switch) if device_switch_requires_restart(&switch) => {
                    tracing::info!(?switch, "device_changed_restarting_source");
...
/// Which default-device switches must tear down and reopen the capture streams.
///
/// Input changes always did. Output changes must too: `capture_settings()` in stream.rs decides
/// AEC and mic isolation from the *current* default output device, but only re-reads it when the
/// source restarts -- there is no other mechanism that notices a mid-recording switch away from
/// headphones. Before this, that switch was silently dropped here (`Ok(_) => {}`), so a session
/// that started isolated stayed marked isolated for its whole duration even after the owner put
/// the headphones down, and any voice the now-shared mic picked up afterwards would have been
/// auto-confirmed as the owner's own. `DeviceListChanged` (e.g. some other device connecting)
/// deliberately does not restart -- it does not change what the *default* output is.
fn device_switch_requires_restart(switch: &DeviceSwitch) -> bool {
    matches!(
        switch,
...
mod tests {
    ...
    #[test]
    fn device_switch_requires_restart_only_for_default_device_changes() {
        assert!(device_switch_requires_restart(
        ...
        assert!(device_switch_requires_restart(
            &DeviceSwitch::DefaultOutputChanged { headphone: None }
        ));
        assert!(!device_switch_requires_restart(
    ...
    #[tokio::test]
    async fn default_output_change_restarts_the_source() {
        // Real end-to-end path: a genuine `DeviceSwitch::DefaultOutputChanged` value -- the
        // exact type the macOS CoreAudio callback sends when the default output device switches
        // mid-recording -- driven through the real `DeviceChangeWatcher::event_loop` against a
        // real spawned `SourceActor`. The only thing stubbed out is the OS-level CoreAudio
        // callback itself, which cannot fire in CI without physically switching a Bluetooth
        // device; everything downstream of that event is exercised as it runs in production.
        //
        // Before this fix, this exact event was silently dropped (`Ok(_) => {}` in
        // `event_loop`), so this test would hang until the timeout and fail.
        let (progress_tx, _progress_rx) = mpsc::unbounded_channel();
        let (capture_tx, _capture_rx) = mpsc::unbounded_channel();
        let audio = Arc::new(TestAudio {
            capture_tx,
        ...
        let (actor, handle) = Actor::spawn(
            ...
            SourceArgs {
                ...
                audio,
                session_id: "output-switch-test".to_string(),
                listener_routing: ListenerRouting::Dropped,
                recorder: None,
            },
        )
        ...
        event_tx
            .send(DeviceSwitch::DefaultOutputChanged {
                headphone: Some(false),
            })
            .unwrap();
        drop(event_tx);
        std::thread::spawn(move || DeviceChangeWatcher::event_loop(event_rx, actor));

        tokio::time::timeout(std::time::Duration::from_secs(2), handle)
            .await
            .expect("source actor did not restart after the default output device changed")
            .unwrap();
    }
}
