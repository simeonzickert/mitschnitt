...
mod tests {
    ...
    // Diese drei Tests haengen am Datei-Handwerk des Aufnahme-Senders: was
    // passiert mit einem bereits vorhandenen MP3, mit einem veralteten WAV
    // daneben, und mit einem Alt-WAV ohne MP3. Der INHALT ist dabei egal --
    // deshalb tragen sie synthetische Sprache genauso wie eine echte Aufnahme.
    #[test]
    ...
    fn create_disk_sink_prefers_existing_mp3_over_stale_wav() {
        ...
        let encoded_path = session_dir.join(FINAL_AUDIO_FILE);
        let wav_path = session_dir.join(WAV_FILE);
        std::fs::copy(anlg_fixtures::SPEECH_DE_MP3, &encoded_path).unwrap();
    ...
    fn create_disk_sink_keeps_legacy_wav_for_append() {
        ...
        let session_dir = dir.path().join("session");
        std::fs::create_dir_all(&session_dir).unwrap();
        std::fs::copy(anlg_fixtures::SPEECH_DE_WAV, session_dir.join(WAV_FILE)).unwrap();
