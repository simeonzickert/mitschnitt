...
use owhisper_client::{
    ...
    DeepgramFluxAdapter, ElevenLabsAdapter, FireworksAdapter, GladiaAdapter,
    GoogleGenerativeAiAdapter, LocalServerAdapter, MistralAdapter, OpenAIAdapter,
    RealtimeSttAdapter, SonioxAdapter, XaiAdapter, anlg_ws_client,
};
...
use super::{
    ChannelSender, ListenerArgs, ListenerMsg, actor_error, actor_error_with_degraded,
...
> {
    ...
    // `removed:` -- Kennungen, fuer die dieser Fork keinen Netz-Client mehr
    // baut (Grok-Review 02.09.2026, F4). `AnarlogAdapter` implementiert
    // `RealtimeSttAdapter` nicht mehr; ein Rueckbau in die Adapter-Liste waere
    // ein Typfehler, und der erschoepfende Match verlangt den Eintrag hier.
...
mod tests {
    ...
    fn listener_args(base_url: &str, model: &str) -> ListenerArgs {
        ListenerArgs {
            ...
            // MicAndSpeaker als Fixture-Default, weil die bestehenden Tests hier den
            // ferne-Teilnehmer-Zweig pruefen und ihn nicht extra ueberschreiben.
            mode: crate::actors::ChannelMode::MicAndSpeaker,
