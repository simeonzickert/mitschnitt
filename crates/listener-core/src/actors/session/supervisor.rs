...
fn update_requires_listener_refresh(current: &SessionParams, update: &SessionConfigUpdate) -> bool {
    // Die Aktualisierung aendert `onboarding` nicht, deshalb bestimmt current.onboarding
    // die Kanal-Lage fuer BEIDE Seiten des Vergleichs.
    let mode = crate::actors::ChannelMode::determine(current.onboarding);
...
fn expected_speaker_count(
    ...
    crate::expected_speakers_per_channel(participant_human_ids, self_human_id, mode)
}
