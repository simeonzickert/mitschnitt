...
/// Hiess vorher `clamps_provider_speakers_to_participant_count` und schrieb
/// fest, dass ein Anbieter-Sprecher jenseits der Teilnehmerzahl auf den
/// letzten erlaubten faellt. Umgeschrieben am 22.09.2026: die Obergrenze ist
/// ausgebaut, weil sie zwei Menschen dauerhaft in die GESPEICHERTEN Wortdaten
/// zusammenlegte. Was bleibt, ist die Untergrenze.
#[test]
fn keeps_high_provider_speakers_apart() {
    let mut first = word("speaker-one", 0.0, 0.5);
    first.speaker = Some(1);
    let mut second = word("speaker-two", 0.5, 1.0);
    ...
    normalize_response_speaker_indices(&mut response);
    ...
    // Frueher wurden beide zu Some(1) -- zwei Menschen unter einem Etikett.
    assert_eq!(words[0].speaker, Some(1));
    assert_eq!(words[1].speaker, Some(2));
    assert_ne!(words[0].speaker, words[1].speaker);
}

/// Hiess vorher `clamps_single_remote_speaker_to_zero`. Auch hier bleibt der
/// hohe Index stehen; nur ungueltige negative Werte werden gefangen.
#[test]
fn keeps_high_speaker_but_catches_negative() {
    let mut high_word = word("too-high", 0.0, 0.5);
    high_word.speaker = Some(7);
    ...
    assert_eq!(words[0].speaker, Some(7), "kein Deckel mehr");
    assert_eq!(words[1].speaker, Some(0), "negativ ist kein Sprecher");
}
