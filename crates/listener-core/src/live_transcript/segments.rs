    ...
    fn state() -> RenderedSegmentState {
        RenderedSegmentState::new(
            ...
            SegmentBuilderOptions {
                ...
                // Wie im Betrieb: der Live-Pfad bekommt seine Optionen aus
                // `segment_options_for_participants`, also mit Deckel. Die
                // Tests hier messen die Schranken der GESAMTEN gerenderten
                // Flaeche (`state.words`), nicht die Blockzahl -- der Deckel
                // aendert an ihren Zusagen nichts.
                max_segment_words: Some(512),
                max_segment_ms: Some(300_000),
                // `..Default::default()` statt Feld fuer Feld: der Live-Pfad
                // gibt die Mehrsprecher-Menge nicht vor, er laesst sie
                // ausrechnen. Ein Literal ohne `..` zwingt jedes neue Feld
                // hier durch -- und bricht genau dann, wenn niemand hinsieht,
                // weil es in `#[cfg(test)]` steht und `cargo build` durchlaeuft.
                ..Default::default()
            },
        )
    }
