...
impl LiveTranscriptEngine {
    ...
    pub fn process(&mut self, response: &StreamResponse) -> Option<LiveTranscriptUpdate> {
        ...
        normalize_response_speaker_indices(&mut normalized);
...
/// Faengt ungueltige Sprecherindizes ab, BEGRENZT sie aber nicht mehr nach
/// oben.
///
/// Fork (22.09.2026): hier stand `speaker.clamp(0, max_speaker_index)`, also
/// dieselbe Deckel-Klasse wie im Renderer -- nur eine Schicht frueher und
/// damit schlimmer: was hier zusammenfaellt, wird SO GESPEICHERT und ist
/// spaeter nicht mehr zu trennen. Bei zwei fernen Teilnehmern wurden die
/// Anbieter-Sprecher 1 und 2 beide zu 1; zwei Menschen unter einem Etikett,
/// dauerhaft, in den Wortdaten.
///
/// Die Untergrenze bleibt: ein negativer Index ist kein Sprecher, sondern
/// Datenmuell, und 0 ist dort die Wahl, die nichts erfindet.
///
/// Der kosmetische Zweck der Obergrenze entfaellt damit ersatzlos und
/// bewusst -- siehe den Kopfkommentar von `SpeakerLabeler` im
/// transcript-Crate.
fn normalize_response_speaker_indices(response: &mut StreamResponse) {
    ...
    for alternative in &mut channel.alternatives {
        for word in &mut alternative.words {
            if let Some(speaker) = word.speaker {
                word.speaker = Some(speaker.max(0));
            }
        }
    }
}
