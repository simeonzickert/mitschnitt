...
private enum LoadedSpeechModel {
  ...
  func transcribe(audio: [Float], sampleRate: Int, language: String?) throws -> String {
    try transcribeWithWordTimings(audio: audio, sampleRate: sampleRate, language: language).text
  }

  /// Fork (02.09.2026): derselbe Aufruf, aber mit den Wortzeiten, die der
  /// Parakeet-Dekoder ohnehin kennt (siehe `src/word_frames.rs`). Alle anderen
  /// Modelle liefern `nil` -- fuer sie bleibt es beim bisherigen Verhalten.
  func transcribeWithWordTimings(audio: [Float], sampleRate: Int, language: String?) throws -> (
    text: String, words: [ParakeetVocabulary.WordTiming]?
  ) {
    ...
    case .streaming(let model):
      return (try model.transcribeAudio(audio, sampleRate: sampleRate), nil)
    case .parakeetBatch(let model, _):
      let text = try model.transcribeAudio(audio, sampleRate: sampleRate, language: languageHint)
      return (text, model.lastWordTimings)
    case .omnilingual(let model):
      return (try model.transcribeAudio(audio, sampleRate: sampleRate), nil)
    case .qwen3(let model):
      // Qwen3's MLX path is non-throwing and takes the language hint directly.
      return (model.transcribe(audio: audio, sampleRate: sampleRate, language: languageHint), nil)
    }
  }
}
...
private func encodeFileTranscriptionJSON(
  ...
  words: [ParakeetVocabulary.WordTiming]? = nil,
  error: String? = nil
) -> String {
  ...
  // Fork: gemessene Wortzeiten, relativ zum Anfang der uebergebenen Datei.
  // Fehlen sie (anderes Modell, nicht passende Frame-Liste), bleibt das Feld
  // weg -- die Rust-Seite faellt dann auf die Gleichverteilung zurueck.
  if let words {
    payload["words"] = words.map { word in
      [
        "text": word.text,
        "startSeconds": Double(word.startTime),
        "endSeconds": Double(word.endTime),
        // Fork (03.09.2026): die Sicherheit, die der Dekoder dem Wort gegeben
        // hat. Bis hierher stand auf der Rust-Seite sechsmal eine harte 1,0 --
        // eine Zahl, die nie etwas gemessen hat.
...
private func encodeDiarizationJSON(_ result: DiarizationResult?, error: String? = nil) -> String {
  var payload: [String: Any] = [
    ...
    // Fork (10.09.2026): die Stimm-Schwerpunkte reisen mit.
    //
    // Das Verfahren liefert je Sprecher eine 256-stellige WeSpeaker-Einbettung
    // (`DiarizationResult.speakerEmbeddings`, der Schwerpunkt seines Clusters).
    // Diese Funktion hat sie bis heute bekommen und liegen lassen -- Rust sah
    // nur Abschnitte und Sprechernummern.
    //
    // Gebraucht werden sie, um einen Kanal in Abschnitten trennen zu koennen:
    // jeder Lauf nummeriert seine Sprecher bei null beginnend neu, und ohne
    // ein Mass fuer "dieselbe Stimme" laesst sich Abschnitt zwei nicht an
    // Abschnitt eins anschliessen. Ordnung und Anzahl entsprechen den
    // `speakerIndex`-Werten oben.
    //
    // Rein additiv: wer den Schluessel nicht liest, merkt nichts.
    "speakerEmbeddings": result?.speakerEmbeddings.map { embedding in
...
private actor SoniqoBridge {
  ...
  func diarizeAudioJSON(modelId: String, samplesData: Data, exactSpeakers: String) async -> String {
    do {
      ...
      // Fork (07.09.2026): "auto" laesst die Sprecherzahl vom Verfahren
      // bestimmen (`Community1SpeakerBounds.inferred`). Vorher war eine exakte
      // Zahl Pflicht -- und weil der Rust-Weg sie nur bei gesetzter
      // Teilnehmerliste mitgab, lief die Trennung bei einer Raumaufnahme ohne
      // Liste ueberhaupt nicht. Die leere Zeichenkette wird gleich behandelt,
      // damit ein fehlender Wert nicht als Fehler durchschlaegt.
      let requested = exactSpeakers.trimmingCharacters(in: .whitespacesAndNewlines)
      let bounds: Community1SpeakerBounds
      if requested.isEmpty || requested.lowercased() == "auto" {
        bounds = .inferred
      } else {
        guard let speakerCount = Int(requested), speakerCount >= 2 else {
          throw SoniqoBridgeError.message("Soniqo diarization requires at least two speakers.")
        }
        bounds = Community1SpeakerBounds(exact: speakerCount)
      }
  ...
  func transcribeAudioFileJSON(modelId: String, audioPath: String, language: String) async -> String
  {
    do {
      ...
      defer { markModelIdle(kind) }
      let transcription = try transcribeFileAudio(
        model: model,
        kind: kind,
        audio: audio,
        sampleRate: soniqoFileTranscriptionSampleRate,
  ...
  private func transcribeFileAudio(
    ...
    sampleRate: Int,
    language: String?
  ) throws -> (text: String, words: [ParakeetVocabulary.WordTiming]?) {
    ...
    var chunks: [String] = []
    // Fork: die Wortzeiten eines Abschnitts zaehlen ab dessen Anfang. Ohne den
    // Versatz laege alles ab dem zweiten Abschnitt am Dateianfang. Sobald ein
    // Abschnitt keine Zeiten liefert, sind sie fuer die ganze Datei hin -- eine
    // halbe Zeitachse waere schlimmer als keine.
    var words: [ParakeetVocabulary.WordTiming]? = []
    ...
    for range in ranges {
      ...
      if let chunkWords = chunk.words, words != nil {
        let offset = Float(range.lowerBound) / Float(sampleRate)
        words?.append(
          contentsOf: chunkWords.map {
            ParakeetVocabulary.WordTiming(
              text: $0.text, startTime: $0.startTime + offset, endTime: $0.endTime + offset,
    ...
    return (chunks.joined(separator: " "), words)
  }
  ...
  private func transcribeFileAudioChunk(
    ...
    // Die Polsterung auf die Mindestlaenge haengt STILLE HINTEN an
    // (`normalizedFileTranscriptionAudio`) -- sie verschiebt keine Zeitmarke.
    let normalizedAudio = normalizedFileTranscriptionAudio(
      kind: kind,
      audio: audio,
      sampleRate: sampleRate
    )
    return try model.transcribeWithWordTimings(
      audio: normalizedAudio, sampleRate: sampleRate, language: language)
  }
