...
use crate::{
    Diarization, DiarizationSegment, Error, FileTranscript, LivePartial, ModelDownloadState,
...
struct FileTranscriptionPayload {
    ...
    /// Fork: gemessene Wortzeiten. Fehlt das Feld, liefert dieses Modell keine.
    #[serde(default)]
    words: Vec<crate::TranscriptWord>,
    error: Option<String>,
}
...
struct DiarizationPayload {
    ...
    num_speakers: usize,
    /// Stimm-Schwerpunkte je Sprecher. `default`, damit eine Bruecke ohne
    /// diesen Schluessel weiterhin liest, statt den ganzen Lauf an einem
    /// fehlenden Feld scheitern zu lassen.
    #[serde(default)]
    speaker_embeddings: Vec<Vec<f32>>,
    error: Option<String>,
}
...
pub(crate) fn diarize_samples(
    ...
    // `None` heisst "Sprecherzahl selbst bestimmen"; die Swift-Seite erkennt
    // "auto" und nimmt dann `Community1SpeakerBounds.inferred`.
    let exact_speakers_value = sr_string(
        &exact_speakers
    ...
    // Nur eine VORGEGEBENE Zahl ist eine Zusage, die gebrochen werden kann.
    // Bei `None` ist die zurueckgegebene Zahl das Ergebnis, nicht die Vorgabe.
    if let Some(exact_speakers) = exact_speakers
