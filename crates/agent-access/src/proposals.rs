...
// Grosszuegig fuer jede echte Zusammenfassung oder jedes echte Memo (die
// laengsten realen Faelle liegen im niedrigen Kilobyte-Bereich), eine Grenze
// fuer einen Unfall (z. B. ein ganzes Transkript, das versehentlich als
// Inhalt mitgeschickt wird). Der Inhalt wird kopiert, in der Datenbank
// gespeichert und zusammen mit dem berechneten Diff mehrfach zurueckgegeben
// (create, list, get) -- ohne Obergrenze sprengt ein einzelner Aufruf sowohl
// den Speicher dieses Prozesses als auch die Datenbank auf der Platte.
pub const MAX_PROPOSAL_CONTENT_BYTES: usize = 200_000;
...
pub struct CreateProposalInput {
    #[schemars(description = "Mitschnitt meeting id")]
...
pub async fn create_proposal(pool: &SqlitePool, input: CreateProposalInput) -> Result<Proposal> {
    ...
    if content.len() > MAX_PROPOSAL_CONTENT_BYTES {
        return Err(Error::Invalid(format!(
            "proposal content is {} bytes, over the {MAX_PROPOSAL_CONTENT_BYTES} byte limit",
            content.len()
        )));
    }
...
mod tests {
    ...
    // Ein einzelner Aufruf darf weder die Datenbank noch den Prozess-Speicher
    // sprengen: Inhalt ueber der Obergrenze wird abgelehnt, Inhalt genau an
    // der Grenze geht noch durch (die Grenze trifft den Normalfall nicht).
    #[tokio::test]
    async fn create_rejects_content_over_the_size_limit_but_accepts_content_at_it() {
        let db = test_db().await;
        seed_meeting(&db).await;

        let too_long = create_proposal(
            db.pool(),
            CreateProposalInput {
                meeting_id: "meeting-1".to_string(),
                kind: "memo".to_string(),
                target_id: None,
                content: "x".repeat(MAX_PROPOSAL_CONTENT_BYTES + 1),
                source: None,
            },
        )
        .await
        .unwrap_err();
        assert!(matches!(too_long, Error::Invalid(_)));

        let at_limit = create_proposal(
            db.pool(),
            CreateProposalInput {
                meeting_id: "meeting-1".to_string(),
                kind: "memo".to_string(),
                target_id: None,
                content: "x".repeat(MAX_PROPOSAL_CONTENT_BYTES),
                source: None,
            },
        )
        .await
        .unwrap();
        assert_eq!(at_limit.proposed_markdown.len(), MAX_PROPOSAL_CONTENT_BYTES);
    }
}
