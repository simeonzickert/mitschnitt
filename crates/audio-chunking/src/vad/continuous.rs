...
use anlg_vad::silero_onnx::CHUNK_SIZE_16KHZ;
...
mod tests {
    ...
    // Der Durchreiche-Beweis: was hinten herauskommt, muss Ton fuer Ton dem
    // entsprechen, was vorne hineinging. Das haengt am Handwerk der Blockung,
    // nicht daran, WAS gesprochen wird -- ein synthetischer Ton traegt es
    // genauso.
    #[tokio::test]
    async fn test_no_audio_drops_for_continuous_vad() {
        let all_audio =
            rodio::Decoder::try_from(std::fs::File::open(anlg_fixtures::SPEECH_DE_WAV).unwrap())
                .unwrap()
                .collect::<Vec<_>>();
    ...
    async fn test_no_speech_drops_for_vad_chunks() {
        ...
        // Der Vorgaenger prueft "mehr als 100 Sekunden Sprache" -- eine Zahl,
        // die nur zu einer dreiminuetigen Fremdaufnahme passte. Der Testton ist
        // acht Sekunden lang und fast durchgehend Sprache; gemessen kommen
        // 120461 von 128525 Abtastwerten zurueck (93,7 %). Die Schranke steht
        // bei 80 %, damit ein echter Sprachverlust auffaellt, kleine
        // Schwankungen der Erkennung aber nicht.
        assert!(
            all_audio_from_vad.len() * 5 > total_samples * 4,
            "nur {} von {total_samples} Abtastwerten kamen als Sprache zurueck",
            all_audio_from_vad.len()
        );
    }
    ...
    async fn test_vad_chunks_are_monotonic_and_non_overlapping() {
        ...
        assert!(!chunks.is_empty(), "keine Sprachbloecke gefunden");
