...
mod tests {
    ...
    // Formatmatrix. Sie haengt am Handwerk (welche Container liest der Dekoder,
    // meldet er Rate und Kanalzahl richtig), nicht am Inhalt -- deshalb traegt
    // sie synthetische Sprache genauso gut wie eine echte Aufnahme.
    ...
    test_audio_file_metadata! {
        test_audio_file_metadata_wav: anlg_fixtures::SPEECH_DE_WAV,
        test_audio_file_metadata_mp3: anlg_fixtures::SPEECH_DE_MP3,
        test_audio_file_metadata_mp4: anlg_fixtures::SPEECH_DE_MP4,
        test_audio_file_metadata_m4a: anlg_fixtures::SPEECH_DE_M4A,
        test_audio_file_metadata_ogg: anlg_fixtures::SPEECH_DE_OGG,
        test_audio_file_metadata_flac: anlg_fixtures::SPEECH_DE_FLAC,
        test_audio_file_metadata_aac: anlg_fixtures::SPEECH_DE_AAC,
        test_audio_file_metadata_aiff: anlg_fixtures::SPEECH_DE_AIFF,
        test_audio_file_metadata_caf: anlg_fixtures::SPEECH_DE_CAF,
    }

    // Der Vorgaenger prueft nur `> 0`. Eine Datei mit zwei Kanaelen darf hier
    // nicht als mono durchgehen, sonst faengt die Matrix genau den Fehler
    // nicht, um den es beim Stereo-Erhalt geht.
    #[test]
    fn test_audio_file_metadata_reports_stereo_as_stereo() {
        let metadata = audio_file_metadata(anlg_fixtures::SPEECH_STEREO_MP3).unwrap();
