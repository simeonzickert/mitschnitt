// Mitschnitt-Fork (02.09.2026): als Kommandozeilen-Werkzeug im Fork ohne
// Release-Pipeline stillgelegt. Der Pruefer verlangt ein Manifest aus
// .github/workflows/desktop_cd.yaml (SOURCE_WORKFLOW) und eine Provenienz
// fuer "fastrepl/hyprnote2" (APPLICATION) -- beides gibt es hier nicht. Die
// exportierten Funktionen bleiben, damit ihre Tests (node --test) weiter die
// Schwellen in scripts/qa/linux-audio-qa-policy.json absichern; der
// Direktaufruf meldet die Stilllegung und beendet sich mit Code 2.
...
if (isMain) {
  console.error(
    "verify-linux-audio-qa-evidence.mjs: im Mitschnitt-Fork stillgelegt (keine Release-Pipeline, desktop_cd.yaml existiert nicht).",
  );
  process.exit(2);
  // eslint-disable-next-line no-unreachable -- Upstream-Weg, bewusst hinter der Stilllegung belassen.
  main().catch((error) => {
