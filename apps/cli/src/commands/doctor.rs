...
struct DatabaseReport {
    path: PathBuf,
    // Welche Regel den Pfad geliefert hat. Ohne diese Zeile sieht ein falscher
    // Ordner genauso aus wie eine fehlende Datenbank.
    source: &'static str,
    // Gefuellt, wenn ein Hinweis der App da war und verworfen wurde. Genau das
    // Ereignis, fuer das die Pruefung existiert -- es darf nicht stumm sein.
    rejected_hint: Option<String>,
    exists: bool,
...
async fn inspect(args: &Args) -> Result<DoctorReport> {
    let source = db::resolve_source(args)?;
    let path = source.path().to_path_buf();
    let exists = path.exists();
    let mut database = DatabaseReport {
        path: path.clone(),
        source: source.label(),
        rejected_hint: source.rejected_hint().map(str::to_string),
        exists,
...
async fn schema_check(db: &anlg_db_core::Db) -> std::result::Result<(), String> {
    tokio::try_join!(
        anlg_db_app::get_session(db.pool(), "__mitschnitt_doctor__"),
        anlg_db_app::list_session_documents(db.pool(), "__mitschnitt_doctor__"),
        anlg_db_app::list_session_transcripts(db.pool(), "__mitschnitt_doctor__"),
        anlg_db_app::list_session_participants(db.pool(), "__mitschnitt_doctor__"),
        anlg_db_app::list_session_action_items(db.pool(), "__mitschnitt_doctor__"),
...
fn render(report: &DoctorReport) -> String {
    ...
    let mut lines = vec![
        format!("Mitschnitt CLI {}", report.cli_version),
        format!("Ready: {}", status(report.ready)),
        format!("Database: {}", report.database.path.display()),
        format!("Resolved from: {}", report.database.source),
    ...
    if let Some(reason) = &report.database.rejected_hint {
        lines.push(format!("Discarded hint: {reason}"));
    }
