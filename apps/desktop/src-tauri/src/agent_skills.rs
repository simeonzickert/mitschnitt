...
// Installs into ~/.claude/skills/<name>/ and the equivalents for Codex, Cursor
// and opencode, overwriting unconditionally. Under the upstream name this fork
// would have shared that directory with a productive anarlog install -- inside
// the user's own agent configuration.
const SKILL_DIR_NAME: &str = "mitschnitt";

// The published skill package from `skills/mitschnitt`, embedded at build time
// so installs work offline and always match the running app version.
//
// `references/mcp.md` ist entfallen: dieser Fork hat keinen MCP-Dienst mehr
// (`apps/cli/src/cli.rs` haelt per Test fest, dass `mcp`, `meetings` und
// `auth` aus der Befehlsflaeche verschwunden sind).
const SKILL_FILES: &[(&str, &str)] = &[
    (
        "SKILL.md",
        include_str!("../../../../skills/mitschnitt/SKILL.md"),
    ),
    (
        "references/cli.md",
        include_str!("../../../../skills/mitschnitt/references/cli.md"),
    ),
    (
        "references/errors.md",
        include_str!("../../../../skills/mitschnitt/references/errors.md"),
    ),
    (
        "references/setup.md",
        include_str!("../../../../skills/mitschnitt/references/setup.md"),
    ),
];
...
fn home_dir() -> Result<PathBuf, String> {
    dirs::home_dir().ok_or_else(|| "Mitschnitt could not find your home directory.".to_string())
}
