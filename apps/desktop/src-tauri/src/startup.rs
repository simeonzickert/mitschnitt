...
pub fn exit_for_already_running_instance() -> ! {
    eprintln!("another Mitschnitt process holds the launch lock; exiting");

    #[cfg(target_os = "macos")]
    {
        let alert = "display alert \"Mitschnitt is already starting\" message \"Another Mitschnitt process is preparing your data, possibly finishing an update. The app will open automatically when it is ready.\" buttons {\"OK\"} default button \"OK\"";
...
fn spawn_indicator_alert() -> Option<std::process::Child> {
    let alert = "display alert \"Updating your data\" message \"Mitschnitt is updating your data. This can take several minutes for a large library.\\n\\nPlease keep Mitschnitt running; it will open automatically when the update finishes.\" buttons {\"OK\"} default button \"OK\"";
