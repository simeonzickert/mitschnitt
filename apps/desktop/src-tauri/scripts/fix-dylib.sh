...
#
# ACHTUNG, gemessen 22.09.2026: auf macOS tut dieses Skript derzeit NICHTS, und
# zwar baulich, nicht zufaellig. `before-bundle.mjs` ruft es ohne Argument auf,
# also ohne Bundle-Ordner, und es steigt in Zeile "No bundle dir provided" aus.
# Das koennte es auch gar nicht anders: als beforeBundleCommand laeuft es VOR
# dem Bundler, das .app existiert zu dem Zeitpunkt noch nicht. Wer hier einen
# Dylib-Fixer vermutet, verlaesst sich auf eine Attrappe.
#
# libmp3lame (seit 22.09. dynamisch, LGPL) braucht es deshalb nicht: die
# Bibliothek traegt ihren Ladenamen @rpath/... schon aus dem Bau
# (vendor/mp3lame-sys/build.rs), der Bundler legt sie ueber tauri.conf
# macOS.files nach Contents/Frameworks, und den LC_RPATH setzt
# .cargo/config.toml beim Linken. Kein Schritt davon braucht eine
# Nachbearbeitung am fertigen Bundle.
