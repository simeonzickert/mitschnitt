...
async function mutateSettingList<T>(
  ...
  // Own queue key on purpose, not "app-settings" (D3, Fix-Runde 1d): this
  // touches only its own list row and protects it with a compare-and-set on
  // value_json plus retries, so it cannot clobber -- or be clobbered by --
  // the start-up migration, which never writes these rows. Sharing the
  // global key would only serialise event ignores behind every other
  // settings write.
