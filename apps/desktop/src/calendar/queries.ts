  ...
  >({
    sql: `
      SELECT
        ...
        locked,
        -- Nur die zwei Felder, nie das ganze metadata_json: die Liste laedt
        -- jedes Gespraech, und der Rest der Metadaten wird hier nie gebraucht.
        -- Beide Import-Formen, sonst stuenden die per Datei importierten
        -- Gespraeche ohne Merker da (siehe imports/provenance.ts).
        CASE WHEN json_valid(metadata_json) THEN COALESCE(
          json_extract(metadata_json, '$.import.source_app'),
          json_extract(metadata_json, '$.importedFrom')
        ) END AS import_source_app,
        CASE WHEN json_valid(metadata_json)
          THEN json_extract(metadata_json, '$.import.imported_at')
        END AS import_imported_at
      FROM sessions
