...
export type MeetingImportProvider = {
  id: string;
  name: string;
  access: "API" | "CLI" | "Export" | "Folder" | "MCP" | "OAuth" | "Webhook";
  helpUrl: string;
  directImport?: "cli" | "mcp-oauth";
  /**
   * Eigener Ablauf mit Ordner-Auswahl statt Dateiauswahl. Der Textdatei-Weg in
   * `parser.ts` kennt nur Sprecher, Text und Zeitspanne und wuerde Woerter,
   * Kanaele und Ton wegwerfen -- fuer eine Quelle mit eigener Datenbank ist er
   * kein verkleinerter, sondern ein falscher Import.
   */
...
export const MEETING_IMPORT_PROVIDERS: MeetingImportProvider[] = [
  // Der einzige Eintrag, der die fremde Marke im Klartext traegt, und das mit
  // Absicht: der Mensch sucht hier den Namen der App, aus der er kommt. Die
