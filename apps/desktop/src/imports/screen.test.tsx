...
vi.mock("./anarlog-dialog", () => ({
  AnarlogImportDialog: ({ providerName }: { providerName: string }) => (
    <div>Folder import dialog for {providerName}</div>
  ),
}));
...
describe("MeetingImportScreen", () => {
  ...
  it("connects Plaud by running the local CLI instead of file-only import", async () => {
    ...
    expect(
      screen.getByText(
        /Connected · New meetings are imported automatically while Mitschnitt is running/i,
      ),
    ).toBeTruthy();
    expect(
  ...
  it("sends the folder source into its own flow instead of the file picker", async () => {
    mockDetected(["anarlog"]);

    renderImports();

    expect(await screen.findByText("anarlog / Hyprnote")).toBeTruthy();
    // Keine Dateiauswahl fuer diese Quelle: der Textdatei-Weg wuerde Woerter,
    // Kanaele und Ton wegwerfen.
    expect(screen.queryByRole("button", { name: "Choose files" })).toBeNull();
    expect(
      screen.queryByText("Folder import dialog for anarlog / Hyprnote"),
    ).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "Choose folder" }));

    expect(
      await screen.findByText("Folder import dialog for anarlog / Hyprnote"),
    ).toBeTruthy();
  });
