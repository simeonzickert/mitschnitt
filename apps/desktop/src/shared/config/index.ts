...
/**
 * Ein gespeicherter Listenwert als Liste -- die EINE Fassung dieser Regel.
 *
 * Herausgegeben, weil ein Schreiber, der den gespeicherten Stand im Moment des
 * Schreibens liest (`updateSettingValue`), ihn genauso zerlegen muss wie die
 * Anzeige. Eine zweite Fassung daneben waere die Sorte Kopie, die driftet.
 */
export function parseConfigStringList(
  value: unknown,
  fallback: string[] = [],
): string[] {
  return parseStringArray(value, fallback);
}
