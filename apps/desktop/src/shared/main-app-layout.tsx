...
const useNavigationEvents = () => {
  ...
  useMountEffect(() => {
    ...
    // Der Altname bleibt als Zweitschluessel gesetzt: aelter erzeugtes HTML
    // (crates/buffer schrieb den Aufruf frueher fest in Erwaehnungs-Links)
    // ruft ihn weiterhin auf. Beide zeigen auf dieselbe Funktion.
    (window as any).__MITSCHNITT_NAVIGATE__ = navigateFromNative;
    ...
    return () => {
      cancelled = true;
      for (const key of ["__MITSCHNITT_NAVIGATE__", "__ANARLOG_NAVIGATE__"]) {
        if ((window as any)[key] === navigateFromNative) {
