...
function StartupErrorView({ error }: { error: Error }) {
  // Internal, non-user-facing match against the Rust backend's migration error
  // (crates/db-migrate emits "...newer version of Mitschnitt"). This string
  // must track the backend message verbatim: it is a comparison, not a label.
  // Change it only together with crates/db-migrate/src/error.rs and the other
  // consumers, never on its own -- a mismatch fails silently by showing the
  // generic startup error instead of the update prompt.
  const needsUpdate = error.message.includes(
    "created by a newer version of Mitschnitt",
  );
    ...
    >
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <h1 className="text-foreground text-base font-semibold">
          {needsUpdate
            ? "Mitschnitt needs an update"
            : "Mitschnitt could not start"}
        </h1>
        <p className="text-muted-foreground text-sm leading-relaxed">
          {needsUpdate
            ? "Your data was created by a newer version of Mitschnitt, and this older version cannot open it. Your existing data was left unchanged. Please install the latest version of Mitschnitt."
