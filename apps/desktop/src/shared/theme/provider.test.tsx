...
const themeState = vi.hoisted(() => ({
  ...
  appIcon: "default" as "default" | "stable",
}));
...
const getIdentifier = vi.hoisted(() =>
  vi.fn(async () => "media.zickert.mitschnitt.stable"),
);
...
describe("AppThemeProvider", () => {
  beforeEach(() => {
    ...
    getIdentifier.mockResolvedValue("media.zickert.mitschnitt.stable");
  ...
  it("uses the native appearance for the system theme", async () => {
    ...
    // Dieselbe Datei im Dunkelmodus: kein "-dark"-Suffix mehr (Welle ohne
    // freigegebene dunkle Fassung, 02.09.2026).
    await waitFor(() => expect(setDockIcon).toHaveBeenCalledWith("stable"));
    expect(setDockIcon).not.toHaveBeenCalledWith("stable-dark");
  });

  it("uses the channel-specific default Dock icon", async () => {
    getIdentifier.mockResolvedValue("media.zickert.mitschnitt.staging");
    themeState.settingsReady = true;
  ...
  it("keeps the production icon on a dark theme against a light system", async () => {
    themeState.settingsReady = true;
    themeState.theme = "dark";
    themeState.appIcon = "stable";
  ...
  it("applies an icon selection using the system appearance", async () => {
    ...
    await applyAppIconPreference("stable");
  ...
  it("applies an icon selection using an explicit theme", async () => {
    ...
    await applyAppIconPreference("default", "light");

    expect(setDockIcon).toHaveBeenCalledWith("staging");
  });
});
