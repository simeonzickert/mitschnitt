...
export type LiveStartBlockReason =
  ...
  // Only the store-level getLiveStartBlockReason reports this one: the live
  // state alone cannot see a batch transcription, that lives in the batch
  // slice.
  | "running_batch"
...
export type GeneralState = {
  live: {
    ...
    // Which of those post-stop leases belong to a capture RECOVERY run rather
    // than to the normal stop path. A recovery run lives in a component that
    // can be unmounted mid-flight, so its lease has to be releasable from the
    // outside; a normal stop always finishes its own.
    recoveryFinalizationBySession: Record<string, boolean>;
