...
describe("runBatchSession", () => {
  ...
  test("uses synthetic progress only for blocking batch providers", () => {
    ...
    // Die drei Faelle, die hier standen, liefen ueber den Anbieter "am" -- den
    // URL-schnueffelnden Sammel-Anbieter der Argmax-Strecke. Der ist am 01.09.2026
    // entfallen; damit gibt es keinen Anbieter mehr, dessen Fortschrittsart erst aus
    // der Basis-Adresse folgt. Was davon uebrig bleibt und weiter zaehlt: der lokale
    // whisper.cpp-Dienst meldet echten Fortschritt, braucht also keinen erfundenen.
    expect(
      shouldUseSyntheticBatchProgress({
        session_id: "session-1",
        provider: "whispercpp",
