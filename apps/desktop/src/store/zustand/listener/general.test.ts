...
describe("General Listener Slice", () => {
    ...
    test("handleBatchResponseStreamed tracks progress per session", () => {
      ...
      expect(
      ...
      ).toEqual([
        {
          ...
          channel: 0,
          metadata: {
            // Fork (03.09.2026): KEINE `confidence` in den Metadaten, obwohl
            // die Nutzlast oben 0,9 traegt -- und das ist richtig so.
            //
            // Das ist der LIVE-Weg. `stream::Word` hat gar kein Feld fuer eine
            // gemessene Sicherheit; die 0,9 daneben ist der nackte Draht-Wert,
            // und der steht bei jedem Anbieter ohne Messung auf 1,0. Wer ihn
            // hier als Messung uebernaehme, koennte spaeter "sicher gemessen"
            // nicht mehr von "nichts gewusst" unterscheiden.
            //
            // Bekannte Grenze, ausdruecklich: Anbieter, die im Live-Betrieb
            // eine echte Wortsicherheit liefern, verlieren sie dadurch. Die
            // sichere Richtung ist, nichts zu behaupten.
            timing: {
        ...
        [
          {
            ...
            channel: 0,
            metadata: {
              // Fork (03.09.2026): keine `confidence` -- derselbe Grund wie im
              // Test darueber. Der Live-Weg kennt `stream::Word` ohne Feld fuer
              // eine gemessene Sicherheit; die 0,9 in der Nutzlast ist der
              // Draht-Wert, und der steht ohne Messung auf 1,0.
              timing: {
                source: "provider_word",
              },
            },
  ...
  describe("Resume After Stop", () => {
    ...
    test("aborts a running batch and reports only once the mode left running_batch", async () => {
      const sessionId = "session-resume-batch";
      store.getState().handleBatchStarted(sessionId);
      expect(store.getState().getSessionMode(sessionId)).toBe("running_batch");

      let settled = false;
      const pending = store
        .getState()
        .resumeAfterStop(sessionId)
        .then((outcome) => {
          settled = true;
          return outcome;
        });
      ...
      await vi.waitFor(() =>
        expect(stopTranscriptionMock).toHaveBeenCalledWith(sessionId),
      );
      await settle();
      expect(settled).toBe(false);
      expect(store.getState().getSessionMode(sessionId)).toBe("running_batch");

      store.getState().handleBatchStopped(sessionId);

      await expect(pending).resolves.toBe("started");
      expect(store.getState().getSessionMode(sessionId)).toBe("inactive");
    });

    test("waits for postStopProcessingBySession to fall before canStartLiveSession is true", async () => {
      const sessionId = "session-resume-poststop";
      expect(store.getState().beginCaptureRecoveryFinalization(sessionId)).toBe(
        true,
      );
      expect(store.getState().canStartLiveSession(sessionId)).toBe(false);
      ...
      await expect(pending).resolves.toBe("started");
      expect(store.getState().canStartLiveSession(sessionId)).toBe(true);
    });

    test("reports blocked instead of hanging when post-stop processing never settles", async () => {
      const sessionId = "session-resume-timeout";
      store.getState().beginCaptureRecoveryFinalization(sessionId);
    ...
    test("reports blocked immediately while another session is recording", async () => {
      store.setState((state) =>
        mutate(state, (draft) => {
          draft.live.status = "active";
          draft.live.sessionId = "session-other";
        }),
      );

      await expect(
        store.getState().resumeAfterStop("session-resume-foreign"),
      ).resolves.toBe("blocked");
      expect(stopTranscriptionMock).not.toHaveBeenCalled();
    });

    test("keeps asking the batch to stop while it is still registered as running", async () => {
      const sessionId = "session-resume-late-registry";
      store.getState().handleBatchStarted(sessionId);
      // The first stop lands in the window between handleBatchStarted and the
      // native registry entry, so it does nothing at all.
      stopTranscriptionMock.mockImplementation(async () => {
        if (stopTranscriptionMock.mock.calls.length >= 2) {
          store.getState().handleBatchStopped(sessionId);
        }
      ...
      await expect(
        store.getState().resumeAfterStop(sessionId, { timeoutMs: 3_000 }),
      ).resolves.toBe("started");
      expect(stopTranscriptionMock.mock.calls.length).toBeGreaterThanOrEqual(2);
    });

    test("abandonCaptureRecoveryFinalization frees a lease that outlived its run", () => {
      const sessionId = "session-orphaned-lease";
      expect(store.getState().beginCaptureRecoveryFinalization(sessionId)).toBe(
        true,
      );
      expect(store.getState().canStartLiveSession(sessionId)).toBe(false);
      ...
      expect(
        store.getState().abandonCaptureRecoveryFinalization(sessionId),
      ).toBe(true);
      expect(
        store.getState().live.postStopProcessingBySession[sessionId],
      ).toBeUndefined();
    ...
    test("a straggling finish without a lease touches neither the stop nor a running start", () => {
      const sessionId = "session-straggler";
      store.setState((state) =>
        mutate(state, (draft) => {
          // A normal stop is still doing its post-processing, and the session
          // is meanwhile starting again (markLiveStartRequested state).
          draft.live.postStopProcessingBySession[sessionId] = true;
          draft.live.sessionId = sessionId;
          draft.live.loading = true;
    ...
    test("abandonCaptureRecoveryFinalization leaves a normal stop's post-processing alone", () => {
      const sessionId = "session-normal-stop";
      store.setState((state) =>
        mutate(state, (draft) => {
          draft.live.postStopProcessingBySession[sessionId] = true;
        }),
    ...
    test("getLiveStartBlockReason exposes why a resume was refused", () => {
      const sessionId = "session-resume-reason";
      expect(store.getState().getLiveStartBlockReason(sessionId)).toBeNull();

      store.getState().beginCaptureRecoveryFinalization(sessionId);

      expect(store.getState().getLiveStartBlockReason(sessionId)).toBe(
