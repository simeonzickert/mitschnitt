...
const _PROVIDERS = [
  {
    disabled: false,
    id: "anarlog",
    displayName: "Mitschnitt",
    badge: "Recommended",
    builtIn: true,
    icon: <MitschnittProviderIcon />,
    // Kein Ziel mehr: die Adresse war `VITE_API_URL` + "/stt", also die
    // Gegenstelle des Originals. Der Eintrag bleibt, weil "anarlog" auf dem
    // Draht die Sammel-Kennung fuer die lokalen Modelle ist (stt/capabilities.ts,
    // useRunBatch.ts) und gespeicherte Einstellungen ihn noch tragen; aus der
    // Auswahl ist er gefiltert (siehe unten).
    baseUrl: "",
    models: ["cloud"],
    requirements: [],
  },
  ...
  //
  // Die Whisper-Modelle liefen bisher NUR ueber "Local file", also mit einer
  // ggml-Datei, die der Nutzer selbst besorgt. Als eigener Anbieter bekommt
  // Whisper large-v3-turbo denselben Weg wie Parakeet und Apple Speech:
  // Auswahl, Groesse, Download-Fortschritt.
  //
  // "Batch only": das Modell laeuft bewusst NICHT live mit. Das ist keine
  // Einschraenkung, die wir uns einhandeln, sondern der Fall, den der Betreiber fuehrt
  // -- transkribiert wird nach dem Meeting.
  {
    disabled: false,
    id: "whispercpp",
    displayName: "Whisper",
...
// Fork: the hosted "anarlog" cloud transcription provider is not offered as a
// selectable option. Local providers (soniqo/apple_speech/whisper/parakeet) and
// the other cloud providers stay. The id remains in the ProviderId union below
// so existing provider-id comparisons across the codebase keep type-checking.
export const PROVIDERS = sortProviders(
  // `: boolean` return annotation prevents TS 5.5+ from inferring a narrowing
  // type predicate here — the ProviderId union (and existing provider-id
  // comparisons elsewhere) must stay intact; only the runtime list is filtered.
  _PROVIDERS.filter((provider): boolean => provider.id !== "anarlog"),
  PROVIDER_ORDER,
);
