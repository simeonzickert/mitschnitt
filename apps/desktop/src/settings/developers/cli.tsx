...
export function buildMcpConfiguration(command: string) {
  return JSON.stringify(
    {
      mcpServers: {
        // Der Server-Name landet woertlich in der Konfiguration, die der
        // Nutzer in Claude/Cursor einfuegt. Er hiess "anarlog", waehrend die
        // Kommandozeile dieses Forks nachweislich "mitschnitt" heisst
        // (apps/desktop/src-tauri/src/embedded_cli.rs) -- die kopierte
        // Konfiguration nannte also ein fremdes Produkt.
        mitschnitt: {
          command,
          args: ["mcp"],
        },
...
function McpRow({ status }: { status: EmbeddedCliStatus | undefined }) {
  const isInstalled = status?.state === "installed";
  const commandName = status?.commandName ?? "mitschnitt";
