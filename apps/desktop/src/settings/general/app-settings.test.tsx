...
describe("AppSettingsView", () => {
  ...
  it("lets switch descriptions use the available row width", () => {
    ...
    expect(
      screen.getByRole("switch", { name: "Start Mitschnitt at login" })
  ...
  it("hides macOS-only Dock controls outside macOS", () => {
    ...
    expect(screen.queryByText("Open Mitschnitt from the menu bar.")).toBeNull();
  ...
  it("toggles automatic updates", () => {
    ...
    expect(
      screen.getByText(/installed the next time Mitschnitt opens/),
    ).toBeTruthy();
  });
  ...
  it("hides direct-distribution controls in App Store builds", () => {
    ...
    expect(
      screen.queryByRole("switch", { name: "Start Mitschnitt at login" }),
    ).toBeNull();
    expect(
