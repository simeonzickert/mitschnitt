...
import { countExpiringSessionAudio } from "~/services/audio-retention";
import { retentionDeadlineAnswered } from "~/services/audio-retention-answers";
          ...
          <div>
            ...
            <AudioSettingsView
              audioRetention={{
                ...
                // The stored row, not the resolved one: a missing or
                // unreadable answer means unarmed, which is the safe reading.
                armed: storedSettings.values.audio_retention_confirmed === true,
                onChange: (value) =>
                  setSettingValues(retentionDeadlineAnswered(value)),
                // Mitschnitt-Fork (F17): a shorter deadline deletes, so the
                // picker shows how many recordings before it applies one.
                countExpiring: (policy) =>
                  countExpiringSessionAudio(normalizeAudioRetention(policy)),
              }}
              microphoneDevice={{
