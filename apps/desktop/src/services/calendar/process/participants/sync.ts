...
function computeSessionParticipantChanges({
  ...
  for (const participant of eventParticipants) {
    // The calendar lists the owner among the attendees. Matching by email
    // alone misses them whenever their own contact carries no address or a
    // different one, and the app then stores a second copy of the owner as a
    // participant of their own meeting - which makes a 1:1 look like a group
    // call and costs the remote speaker their name. The provider already
    // tells us who the owner is, so trust that flag. Providers that leave it
