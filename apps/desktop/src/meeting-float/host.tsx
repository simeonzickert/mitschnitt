...
import { startFloatingTrackLevelBroadcast } from "./track-levels";
import {
...
function FloatingMeetingWindowSync({
  ...
  useMountEffect(() => {
    ...
    // The bar's own state carries only the two tracks collapsed into one
    // amplitude; the dead-track watch needs them apart. See ./track-levels.
    const stopTrackLevelBroadcast = startFloatingTrackLevelBroadcast();

    return () => {
      cancelled = true;
      refreshSettingsRef.current = () => {};
      unsubscribe();
      stopTrackLevelBroadcast();
