...
import {
  multiSpeakerChannelsFromSegments,
  SegmentKeyUtils,
  type RenderLabelContext,
  type SegmentChannelProfile,
...
export function getFloatingTranscriptBubbles(
  ...
  // Fork (08.09.2026): die Mehrsprecher-Frage aus den Bloecken beantwortet und
  // in den Kontext gelegt -- `createMeetingFloatLabelContext` kennt die
  // Bloecke nicht und liefert dort "keiner".
  const multiSpeakerChannels = multiSpeakerChannelsFromSegments(segments);
  const ctx: RenderLabelContext | undefined = speakerLabelContext
    ? {
        ...speakerLabelContext,
    ...
    .map((segment) => {
      ...
      return {
        ...
        speakerLabel: getFloatingSpeakerLabel(
          segment.key,
          ctx,
          multiSpeakerChannels,
        ),
        text,
        isSelf: isFloatingSelfSpeaker(segment.key, multiSpeakerChannels),
...
function getFloatingSpeakerLabel(
  ...
  ctx: RenderLabelContext | undefined,
  multiSpeakerChannels: ReadonlySet<SegmentChannelProfile>,
) {
  if (isFloatingSelfSpeaker(key, multiSpeakerChannels)) {
    return "You";
  }
...
/// Ist dieser Block der Nutzer selbst?
///
/// Fork (08.09.2026): der Mehrsprecher-Vorbehalt ergaenzt. Vorher war der
/// Mikrofonkanal ohne jede Bedingung "You" -- auf einer Raumaufnahme trug jede
/// einzelne Blase im Overlay diesen Namen, auch wenn vier Menschen sprachen.
/// Das entscheidet hier zwei Dinge zugleich: das Etikett UND die Ausrichtung
/// der Blase (`isSelf`).
function isFloatingSelfSpeaker(
  key: ListenerState["liveSegments"][number]["key"],
  multiSpeakerChannels: ReadonlySet<SegmentChannelProfile>,
) {
  return key.channel === "DirectMic" && !multiSpeakerChannels.has(key.channel);
}
