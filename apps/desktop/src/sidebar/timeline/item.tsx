...
import { ImportedMarker } from "~/imports/provenance-view";
...
type ItemBaseProps = {
  ...
  // Zwei Skalare statt eines Objekts: der Vergleicher unten prueft auf
  // Identitaet, ein Objekt-Literal wuerde die Memoisierung jeder Zeile toeten.
              ...
              <div className="flex min-w-0 items-center gap-1.5">
                ...
                {importSourceApp ? (
                  <ImportedMarker
                    ...
                    importedAt={importImportedAt ?? null}
                  />
                ) : null}
              </div>
            ) : null}
...
function itemBasePropsAreEqual(prev: ItemBaseProps, next: ItemBaseProps) {
  return (
    ...
    prev.importSourceApp === next.importSourceApp &&
    prev.importImportedAt === next.importImportedAt &&
...
const SessionItem = memo(
  ({
    ...
    const { t } = useLingui();
    const openCurrent = useTabs((state) => state.openCurrent);
    // Mitschnitt-Fork (ISA N8, ZICK-251): the context menu asks before it
    // deletes; the dialog mounts only while a request is pending.
    ...
    const { sessionMode, stop, amplitude, isStartingUp } = useListener(
      (state) => {
    ...
    // starts up or is being transcribed. A native menu has no tooltip, so
    // the reason rides in the label.
    const deleteBlocked = isBusyRecordingMode(sessionMode) || isStartingUp;
    const [isOpening, setIsOpening] = useState(false);
    const showSpinner =
      !selected &&
      !isLive &&
    ...
    }, [requestDelete, sessionId, sessionEvent?.tracking_id, title]);
    ...
    return (
      <>
        ...
        <ItemBase
          ...
          importSourceApp={item.data.import_source_app}
          importImportedAt={item.data.import_imported_at}
          selected={selected}
          muted={muted}
          multiSelected={multiSelected}
          onClick={handleClick}
