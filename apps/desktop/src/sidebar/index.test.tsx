...
vi.mock("./footer-nav", () => ({
  SidebarFooterNav: () => <div data-testid="sidebar-footer-nav" />,
}));
...
describe("LeftSidebar", () => {
  ...
  // Rot, sobald die Fussleiste in einer der Ansichten fehlt oder nicht mehr
  // das letzte Kind der Seitenleiste ist (sie muss unten sitzen, nicht mitten
  // im Inhalt).
  it.each([
    "empty",
    "settings",
    ...
    "templates",
    "automations",
  ])("keeps the footer nav at the bottom in %s mode", (type) => {
    mocks.currentTab = { type };

    const { container } = render(<LeftSidebar />);
    const footer = screen.getByTestId("sidebar-footer-nav");

    expect(container.firstElementChild?.lastElementChild).toBe(footer);
  });
