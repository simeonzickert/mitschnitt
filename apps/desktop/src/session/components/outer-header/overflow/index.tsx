...
export function OverflowButton({
  ...
  // Mitschnitt-Fork (ISA N8, ZICK-251): deleting the note asks first. Same
  // choreography as the export modal -- close the menu, open the dialog on
  // the next frame -- because a dialog opening while the menu is still
  // returning focus fights it over the focus trap.
  ...
  const handleRequestDeleteNote = (
    ...
    setOpen(false);
    requestAnimationFrame(() => requestDelete(targetSessionId, options));
  };
