...
vi.mock("./delete", () => ({
  DeleteNote: ({
    ...
    <button
      type="button"
      onClick={() => onRequestDelete(sessionId, { title: "Planning" })}
    >
      Delete note
    </button>
  ),
...
vi.mock("~/session/hooks/useDeleteSession", () => ({
  useDeleteSession: () => deleteSessionMock,
}));
...
describe("OverflowButton", () => {
  ...
  beforeEach(() => {
    ...
    useListenerMock.mockImplementation((selector) =>
      selector({
        ...
        live: { sessionId: null, loading: false },
        stop: vi.fn(),
      }),
    );
  });
  ...
  // ISA N8 (ZICK-251): "Delete" in the overflow menu asks first. The dialog
  // is rendered by the overflow button, outside the dropdown, and reaches
  // useDeleteSession exactly once -- from the confirm button.
  it("asks before deleting the note and deletes once on confirm", async () => {
    vi.spyOn(window, "requestAnimationFrame").mockImplementation((callback) => {
      callback(0);
      return 0;
    });
    ...
    expect(deleteSessionMock).toHaveBeenCalledExactlyOnceWith("session-1", {
  ...
  it("never deletes the note when the confirmation is cancelled", async () => {
    vi.spyOn(window, "requestAnimationFrame").mockImplementation((callback) => {
      callback(0);
      return 0;
    });
