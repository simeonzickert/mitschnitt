...
export function getTranscriptMergeTarget(
  ...
  // Ein Wort hängt jetzt an den letzten Block DESSELBEN Kanals an, nicht mehr
  // an den unmittelbar letzten Block — zwei Blöcke desselben Kanals sind
  // deshalb in `order` fast nie mehr direkte Nachbarn, meist liegt ein Block
  // des anderen Kanals dazwischen. Zusammenführbar sind daher gleichkanalige
  // ZEIT-Nachbarn: zwischen zwei ausgewählten Einträgen darf im `order`-Array
  // KEIN WEITERER Eintrag DESSELBEN Kanals liegen (Einträge anderer Kanäle
  // dazwischen sind erlaubt und werden übersprungen — indexes ist aufsteigend
  // sortiert, also gibt es zwischen zwei aufeinanderfolgenden Einträgen aus
  // `indexes` per Konstruktion ohnehin keinen weiteren AUSGEWÄHLTEN Eintrag).
  // Ein Zwischen-Eintrag ohne auflösbares `groups[0]` blockiert die
  // Zusammenführung sicherheitshalber (im Zweifel null statt fälschlich
  // Wörter aus unklarem Kontext zusammenzuwerfen).
  for (
    let selectionIndex = 1;
    selectionIndex < indexes.length;
    selectionIndex += 1
  ) {
    const rangeStart = indexes[selectionIndex - 1]! + 1;
    const rangeEnd = indexes[selectionIndex]!;
    for (let between = rangeStart; between < rangeEnd; between += 1) {
      const betweenGroup = entries.get(order[between]!)?.groups[0];
      const blocksMerge =
        !betweenGroup ||
        betweenGroup.segmentKey.channel === targetGroup.segmentKey.channel;
      if (blocksMerge) {
        return null;
      }
    }
