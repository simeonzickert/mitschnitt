...
import { createTranscriptJump } from "./scroll-to-word";
...
export function useVirtualSegments({
  ...
  >(() => {});
  // Der Suchindex wird nur bei geaenderten Segmenten neu gebaut. Damit die
  // Sprungfunktion trotzdem immer den aktuellen Container sieht, laeuft er
  // ueber eine Ref statt ueber die Closure.
  const scrollElementRef = useRef<HTMLDivElement | null>(scrollElement);
        ...
        ? createTranscriptSearchIndex(
            segments.flatMap((segment, index) =>
              segment.words.map((word) => ({
                id: word.id ?? null,
                text: word.text.trim(),
                // Ein zusammengelegter Block kann hoeher sein als das Fenster;
                // dann landet der reine Segment-Scroll am Blockanfang und der
                // Treffer bliebe ungesehen.
                scrollIntoView: createTranscriptJump({
                  scrollToIndex: (ziel, verhalten) =>
                    scrollToIndexRef.current(ziel, verhalten),
