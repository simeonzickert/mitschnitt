...
describe("SpeakerAssignPopover", () => {
  ...
  /**
   * Bei vielen Teilnehmern schnitt das Feld frueher ab: `PopoverContent`
   * TRUG eine eigene `max-h-[min(...),28rem)]`-Deckelung, redundant zur
   * Deckelung, die `SpeakerParticipantPicker` schon selbst auf seinem
   * eigenen Wrapper haelt. Die aeussere Deckelung ist raus -- nur die innere
   * bleibt die einzige Quelle der Hoehenbegrenzung.
   */
  it("deckelt die Popover-Huelle nicht ein zweites Mal auf 28rem", () => {
    ...
    expect(popoverContent?.className).toBe("w-80");
    expect(popoverContent?.className).not.toContain("max-h");

    // Die Ergebnisliste traegt ihre eigene Deckelung weiterhin ueber
    // `SpeakerParticipantPicker`s aeusseren Wrapper (ungeaendert) -- hier
    // wird nur geprueft, dass der SCROLL-Container selbst kein `py-1` mehr
    // traegt (das steht jetzt auf dem inneren Polster-Wrapper mit `pb-3`).
    const aliceOption = screen.getByRole("button", { name: "Alice" });
    const gruppenWrapper = aliceOption.parentElement;
    const polsterWrapper = gruppenWrapper?.parentElement;
    const scrollContainer = polsterWrapper?.parentElement;

    expect(polsterWrapper?.className).toContain("py-1");
    expect(polsterWrapper?.className).toContain("pb-3");
    expect(scrollContainer?.className).toContain("overflow-auto");
    expect(scrollContainer?.className?.split(/\s+/)).not.toContain("py-1");
  });
});
