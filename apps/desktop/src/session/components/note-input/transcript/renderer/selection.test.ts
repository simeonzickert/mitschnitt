...
describe("transcript word selection", () => {
  ...
  it("merges same-channel time-neighbors across a block from the other channel", () => {
    // A(DirectMic) B(RemoteParty) C(DirectMic) in dieser order-Reihenfolge:
    // A und C sind gleichkanalige Zeit-Nachbarn, B liegt dazwischen und wird
    // übersprungen.
    const order = ["a", "b", "c"];
    const entries = new Map([
      ["a", entry("transcript-1", 0, "alice", "word-1", "DirectMic")],
      ["b", entry("transcript-1", 0, "bob", "word-2", "RemoteParty")],
      ["c", entry("transcript-1", 1, "alice", "word-3", "DirectMic")],
    ]);

    expect(canMergeTranscriptEntries(new Set(["a", "c"]), order, entries)).toBe(
      true,
    );
    expect(getTranscriptMergeTarget(new Set(["a", "c"]), order, entries)).toBe(
      entries.get("a"),
    );
  });

  it("refuses to merge across another block of the same channel", () => {
    // A(DirectMic) B(DirectMic) C(DirectMic): zwischen A und C liegt B, ein
    // WEITERER Block desselben Kanals -> keine Zeit-Nachbarschaft.
    const order = ["a", "b", "c"];
    const entries = new Map([
      ["a", entry("transcript-1", 0, "alice", "word-1", "DirectMic")],
      ["b", entry("transcript-1", 1, "alice", "word-2", "DirectMic")],
      ["c", entry("transcript-1", 2, "alice", "word-3", "DirectMic")],
    ]);
  ...
  it("refuses to merge across a channel change", () => {
    const order = ["a", "b"];
    const entries = new Map([
      ["a", entry("transcript-1", 0, "alice", "word-1", "DirectMic")],
      ["b", entry("transcript-1", 0, "bob", "word-2", "RemoteParty")],
...
function entry(
  ...
  wordId: string,
  channel: "DirectMic" | "RemoteParty" | "MixedCapture" = "RemoteParty",
) {
  return {
    text: wordId,
    startMs: 0,
