...
const hoisted = vi.hoisted(() => ({
  ...
  searchValue: null as { isVisible: boolean; close: () => void } | null,
...
vi.mock("./search/bar", () => ({
  SearchBar: ({ allowReplace }: { allowReplace?: boolean }) => (
    <div
      data-testid="search-bar"
      data-allow-replace={String(allowReplace ?? true)}
    />
  ),
}));

vi.mock("./search/context", () => ({
  useSearch: () => hoisted.searchValue,
}));
...
describe("NoteInput tab selection", () => {
  ...
  it("zeigt die Suchleiste im Transkript", () => {
    hoisted.searchValue = { isVisible: true, close: () => {} };
    renderNoteInput({ currentTab: { type: "transcript" } });

    expect(screen.queryByTestId("search-bar")).not.toBeNull();
  });

  it("haelt Ersetzen aus dem Transkript heraus, laesst es in den Notizen", () => {
    hoisted.searchValue = { isVisible: true, close: () => {} };

    renderNoteInput({ currentTab: { type: "transcript" } });
    expect(
      screen.getByTestId("search-bar").getAttribute("data-allow-replace"),
    ).toBe("false");

    cleanup();

    renderNoteInput({ currentTab: { type: "raw" } });
    expect(
      screen.getByTestId("search-bar").getAttribute("data-allow-replace"),
    ).toBe("true");
  });

  it("blendet die Suchleiste aus, solange sie nicht geoeffnet ist", () => {
    hoisted.searchValue = { isVisible: false, close: () => {} };
    renderNoteInput({ currentTab: { type: "transcript" } });

    expect(screen.queryByTestId("search-bar")).toBeNull();
  });

  it("zeigt keine Suchleiste bei den Anhaengen", () => {
    hoisted.searchValue = { isVisible: true, close: () => {} };
    hoisted.editorTabs = [{ type: "transcript" }, { type: "attachments" }];
    renderNoteInput({ currentTab: { type: "attachments" } });
