...
// 200 war zu eng: ein Modell, das vor der Antwort nachdenkt, verbraucht das
// Budget im Denken, die Antwort beginnt nie, und zurueck kommt ein Abbruch
// ohne Fehlermeldung. Dieselbe Falle haben wir am 03.09.2026 an einem anderen
// Modell selbst erlitten (21.358 Denk-Tokens, null Zeichen Antwort, volle
// Rechnung) -- die Lehre stand seither in den Betriebsregeln und ungenutzt
// hier im Code. Ein zu hohes Limit kostet nichts, weil nur erzeugte Tokens
// abgerechnet werden; die Fehlerrichtung ist damit eindeutig.
...
export async function streamPreMeetingBrief({
  ...
  try {
    ...
    // Ein Modell kann den Brief vollstaendig geschrieben und trotzdem kein
    // sauberes Objekt geliefert haben. Dann ist der Rohtext brauchbar --
    // ABER nur, wenn der Lauf nicht am Budget endete: bei `length` ist der
    // Text mittendrin abgeschnitten, und ein halber Brief sieht aus wie ein
    // ganzer. Genau diese Unterscheidung fehlte hier.
    if (
    ...
    // Die Stueckzahl ist der Vollstaendigkeitsbeweis: fehlt ein Punkt, war der
    // Text doch abgeschnitten, egal was der Abbruchgrund sagt.
