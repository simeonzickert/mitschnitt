...
import { buildNoteAttachmentReconcileStatements } from "./note-attachments";
import type {
  SessionChanges,
  SessionRecord,
...
/**
 * The embedded event of one session, for the auto-stop (stt/auto-stop.ts).
 * Reads through getSessionEvent (G4, Fix-Runde 2 -- Forge 5): until this
 * round a raw JSON.parse here bypassed the one place that corrects a legacy
 * welcome session's demo link (E1, session/utils.ts).
 */
export async function loadSessionEvent(
  ...
  const row = rows[0];
  return row ? getSessionEvent(row) : null;
}
...
export function updateSession(
  ...
  return enqueueDatabaseWrite(`session:${sessionId}`, async () => {
    ...
    if (changes.raw_md !== undefined) {
      ...
      // Mitschnitt-Fork (ISA N8): a picture taken out of the note gets its
      // tombstone here, one put back loses it -- same transaction as the
      // note, right after it (see ./note-attachments.ts).
      statements.push(
        ...buildNoteAttachmentReconcileStatements(sessionId, now),
      );
    }
