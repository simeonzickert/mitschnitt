...
vi.mock("~/types/tauri.gen", () => ({
  commands: { mirrorForgetAudio: mocks.mirrorForgetAudio },
}));
...
describe("attachment catalog", () => {
  beforeEach(() => {
    ...
    mocks.mirrorForgetAudio.mockResolvedValue({ status: "ok", data: true });
    ...
    expect(
      statements.every(
        (statement: { sql: string }) =>
          !statement.sql.includes("attachment_transfer_jobs"),
      ),
    ).toBe(true);
  ...
  it("catalogs primary audio with a stable logical identity and root-relative path", async () => {
    ...
    expect(statements[0].sql).toContain("'$.transcript_status'");
  ...
  // Mitschnitt-Fork (F16b). Since F16 the meeting folder carries the recording
  // too -- as a hard link, so clearing the machine room leaves the bytes alive
  // under the other name, and with a mirror root on another volume there is a
  // second file outright. Without this the app says "deleted" while the
  // recording sits in the meeting folder, uploaded to a synced folder already.
  it("takes the recording out of the meeting folder as well", async () => {
    mocks.executeTransaction.mockResolvedValue([1]);
    ...
    expect(mocks.mirrorForgetAudio).toHaveBeenCalledWith("session-1");
  });

  // The same for the retention deadline (Entscheid 01.09.2026): a deadline that
  // deletes only half is a lie. The recording goes from both places; the
  // transcript and the summary in the meeting folder are untouched, which is
  // what `mirrorForgetAudio` does and all it does.
  it("takes the recording from both places when the retention deadline passes", async () => {
    mocks.executeTransaction.mockResolvedValue([1]);

    await deleteLocalSessionAudio("session-1", () => true);

    expect(mocks.audioDelete).toHaveBeenCalledWith("session-1");
    expect(mocks.mirrorForgetAudio).toHaveBeenCalledWith("session-1");
  });

  // The machine-room copy is gone by then; throwing would tell the caller
  // nothing was deleted at all.
  it("still reports a deletion when the meeting folder cannot be cleared", async () => {
    mocks.executeTransaction.mockResolvedValue([1]);
    mocks.mirrorForgetAudio.mockResolvedValue({
      status: "error",
