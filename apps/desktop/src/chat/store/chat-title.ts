...
export async function generateChatTitle({
  ...
  const result = await generateText({
    model,
    maxRetries: 2,
    // 32 reicht fuer sechs Woerter, aber nicht fuer ein Modell, das vor der
    // Antwort nachdenkt: die Decke deckt Denken UND Antwort ab, das Denken
    // frisst sie auf, und zurueck kommt ein leerer Text ohne Fehlermeldung.
    // Siehe title-workflow.ts fuer die Messung dahinter.
