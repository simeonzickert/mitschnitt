  ...
  return (
    <div className="flex h-full flex-1 flex-col">
      {human ? (
        <>
          <ContactPageHeader
            // Fork: der eigene Kontakt heisst "You", nicht "Unnamed". Upstream
            // behandelt ihn wie jeden anderen namenlosen Eintrag, dabei ist er
            // der einzige, den die App sicher identifizieren kann (id ==
            // userId). "You" ist bewusst dasselbe Wort, das der Transkript-
            // Labeler fuer den Mikrofonkanal setzt (crates/transcript label.rs)
            // -- zwei Woerter fuer dieselbe Person waeren eine Verwechslung
            // mehr, und der Fork hat schon genug davon aufgeraeumt.
            title={
              human.name ||
              human.email ||
              (human.id === human.userId ? t`You` : t`Unnamed`)
            }
            compactIdentity={
              human.avatarDataUrl ? (
