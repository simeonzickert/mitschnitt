...
import { isCaptureRecoveryCancelled } from "~/stt/capture-recovery-requests";
...
export function useResumeListeningLifecycle(sessionId: string) {
  ...
  return useCallback(
    async (options?: { abandonOnFailure?: boolean }) => {
      ...
      // from the recording, both of which block or undo the restart. Whenever
      // we bail for that reason, the lease goes back - a run that keeps it
      // while walking away blocks every later start of this session.
      const giveWayToResume = () => {
        ...
        // Reset unconditionally: the lease may already have been broken from
        // the outside, and a ref that still claims ownership would make this
        // run call finish again later, on a lease it no longer holds.
      ...
      // Checked again right before the finalization: a resume can win the race
      // while the awaits above were in flight, and from here on the run would
      // hold postStopProcessing and rewrite the transcript.
      if (giveWayToResume()) {
        return "inactive" as const;
      }
      ...
      try {
        ...
        console.error("[listener] failed to recover stopped capture", error);
        // recoverStopped runs for seconds; a resume that arrived meanwhile
        // must get the lease back, or a failure here would keep the session
        // blocked with nobody left to release it.
        if (giveWayToResume()) {
          return "inactive" as const;
        }
        return failRecovery();
