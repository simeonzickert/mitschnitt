...
describe("formatSegmentStartLabel", () => {
  it("formatiert unter einer Stunde als mm:ss", () => {
    expect(formatSegmentStartLabel(0)).toBe("00:00");
    expect(formatSegmentStartLabel(12_000)).toBe("00:12");
    expect(formatSegmentStartLabel(65_400)).toBe("01:05");
    expect(formatSegmentStartLabel(3_599_000)).toBe("59:59");
  });

  it("nimmt ab einer Stunde die Stunde dazu", () => {
    expect(formatSegmentStartLabel(3_600_000)).toBe("1:00:00");
    expect(formatSegmentStartLabel(3_725_000)).toBe("1:02:05");
  });

  it("gibt null zurueck, wo es keine brauchbare Zeit gibt", () => {
    expect(formatSegmentStartLabel(-1)).toBeNull();
    expect(formatSegmentStartLabel(Number.NaN)).toBeNull();
    expect(formatSegmentStartLabel(Number.POSITIVE_INFINITY)).toBeNull();
  });
});
