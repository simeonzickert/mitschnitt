...
const {
  ...
  abandonCaptureRecoveryFinalizationMock,
  resumeAfterStopMock,
  getLiveStartBlockReasonMock,
  cancelCaptureRecoveryMock,
  isCaptureRecoveryCancelledMock,
  startMock,
  stopMock,
  getSessionModeMock,
  ...
  abandonCaptureRecoveryFinalizationMock: vi.fn(),
  resumeAfterStopMock: vi.fn(),
  getLiveStartBlockReasonMock: vi.fn(),
  cancelCaptureRecoveryMock: vi.fn(),
  isCaptureRecoveryCancelledMock: vi.fn(),
  startMock: vi.fn(),
  stopMock: vi.fn(),
...
vi.mock("~/stt/capture-recovery-requests", () => ({
  ...
  cancelCaptureRecovery: cancelCaptureRecoveryMock,
  isCaptureRecoveryCancelled: isCaptureRecoveryCancelledMock,
}));
...
describe("useStartListening", () => {
  beforeEach(() => {
    ...
    useListenerMock.mockImplementation((selector) =>
      selector({
        ...
        abandonCaptureRecoveryFinalization:
          abandonCaptureRecoveryFinalizationMock,
        resumeAfterStop: resumeAfterStopMock,
        getLiveStartBlockReason: getLiveStartBlockReasonMock,
    ...
    abandonCaptureRecoveryFinalizationMock.mockReturnValue(false);
    resumeAfterStopMock.mockResolvedValue("started");
    getLiveStartBlockReasonMock.mockReturnValue(null);
    cancelCaptureRecoveryMock.mockReturnValue(() => {});
    isCaptureRecoveryCancelledMock.mockReturnValue(false);
  ...
  test("posts the recording disclosure after listening starts when enabled", async () => {
    ...
    await waitFor(() => {
      expect(sendMeetingChatMessageMock).toHaveBeenCalledWith(
        "I'm using Mitschnitt to record and transcribe this meeting.",
  ...
  test("retries until a conferencing app is mic-active without reporting intermediate failures", async () => {
    listMicUsingApplicationsMock
      .mockResolvedValueOnce({
        status: "ok",
        data: [{ id: "com.anarlog.dev", name: "Mitschnitt Dev" }],
      })
      .mockResolvedValueOnce({
        status: "ok",
        data: [{ id: "us.zoom.xos", name: "zoom.us" }],
    ...
    expect(sendMeetingChatMessageMock).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining("I'm using Mitschnitt to record"),
      ["com.anarlog.dev"],
    );
    expect(sendMeetingChatMessageMock).toHaveBeenNthCalledWith(
      2,
  ...
  test("keeps the Slack scope when Mitschnitt also appears in the mic-active apps", async () => {
    useConfigValueMock.mockImplementation((key: string) =>
      key === "ai_language"
        ? "en"
        : key === "consent_auto_send_chat"
    ...
    listMicUsingApplicationsMock.mockResolvedValue({
      status: "ok",
      data: [
        { id: "com.anarlog.dev", name: "Mitschnitt Dev" },
  ...
  test("passes an ambiguous meeting scope for Rust to reject before AX mutation", async () => {
    ...
    expect(sonnerToastWarningMock).toHaveBeenCalledWith(
      "Recording started, but Mitschnitt could not post the meeting chat disclosure.",
      {
        id: "meeting-disclosure-send-failed",
        duration: Infinity,
        description:
          "This needs Accessibility permission. Grant it in System Settings → Privacy & Security → Accessibility.",
      },
    );
    warn.mockRestore();
  });
  ...
  test("a plain start while post-stop processing runs returns silently", async () => {
    canStartLiveSessionMock.mockReturnValue(false);
    getLiveStartBlockReasonMock.mockReturnValue("post_stop_processing");
  ...
  test("useResumeAfterStop starts once the store reports the session unblocked", async () => {
    canStartLiveSessionMock.mockReturnValue(false);
    getLiveStartBlockReasonMock.mockReturnValue("post_stop_processing");
    resumeAfterStopMock.mockImplementation(async () => {
      canStartLiveSessionMock.mockReturnValue(true);
      getLiveStartBlockReasonMock.mockReturnValue(null);
      return "started";
    });

    const { result } = renderHook(() => useResumeAfterStop("session-1"));
    let outcome: Awaited<ReturnType<typeof result.current>> | undefined;
    await act(async () => {
      outcome = await result.current();
    });

    expect(resumeAfterStopMock).toHaveBeenCalledWith("session-1");
    expect(startMock).toHaveBeenCalledOnce();
    expect(outcome).toEqual({ status: "started" });
  });

  test("useResumeAfterStop reports the block reason instead of starting", async () => {
    canStartLiveSessionMock.mockReturnValue(false);
    getLiveStartBlockReasonMock.mockReturnValue("another_session_active");
    resumeAfterStopMock.mockResolvedValue("blocked");
  ...
  test("useResumeAfterStop shields the deliberate resume from capture recovery", async () => {
    const release = vi.fn();
    cancelCaptureRecoveryMock.mockReturnValue(release);
    ...
    expect(cancelCaptureRecoveryMock).toHaveBeenCalledWith("session-1");
    expect(release).toHaveBeenCalledOnce();
    expect(
      cancelCaptureRecoveryMock.mock.invocationCallOrder[0],
    ).toBeLessThan(resumeAfterStopMock.mock.invocationCallOrder[0]!);
    expect(release.mock.invocationCallOrder[0]).toBeGreaterThan(
      startMock.mock.invocationCallOrder[0]!,
    );
  });

  test("useResumeAfterStop retires a stranded marker instead of overwriting it", async () => {
    loadCaptureLifecycleMarkerMock.mockResolvedValue({
      version: 1,
      sessionId: "session-1",
      transcriptId: "transcript-stranded",
      startedAt: 1_000,
    ...
    expect(
      saveCaptureLifecycleMarkerMock.mock.calls[0]?.[0]?.transcriptId,
    ).not.toBe("transcript-stranded");
    expect(
      clearCaptureLifecycleMarkerMock.mock.invocationCallOrder[0],
    ).toBeLessThan(saveCaptureLifecycleMarkerMock.mock.invocationCallOrder[0]!);
    consoleWarn.mockRestore();
  });

  test("a living recovery run keeps its lease: no abandon while the wait succeeds", async () => {
    abandonCaptureRecoveryFinalizationMock.mockReturnValue(true);

    const { result } = renderHook(() => useResumeAfterStop("session-lease-ok"));
    await act(async () => {
      await result.current();
    });

    expect(abandonCaptureRecoveryFinalizationMock).not.toHaveBeenCalled();
    expect(resumeAfterStopMock).toHaveBeenCalledOnce();
    expect(startMock).toHaveBeenCalledOnce();
  });

  test("a dead lease is broken only after the wait reported post-stop processing", async () => {
    resumeAfterStopMock
    ...
    const { result } = renderHook(() =>
      useResumeAfterStop("session-lease-dead"),
    );
    let outcome: Awaited<ReturnType<typeof result.current>> | undefined;
    await act(async () => {
      outcome = await result.current();
    ...
    expect(resumeAfterStopMock).toHaveBeenCalledTimes(2);
    const abandonedAt =
      abandonCaptureRecoveryFinalizationMock.mock.invocationCallOrder[0]!;
    expect(abandonedAt).toBeGreaterThan(
      resumeAfterStopMock.mock.invocationCallOrder[0]!,
    );
    expect(abandonedAt).toBeLessThan(
      resumeAfterStopMock.mock.invocationCallOrder[1]!,
    );
    expect(outcome).toEqual({ status: "started" });
  });

  test("a block for any other reason is reported without a second attempt", async () => {
    resumeAfterStopMock.mockResolvedValue("blocked");
    getLiveStartBlockReasonMock.mockReturnValue("another_session_active");

    const { result } = renderHook(() =>
      useResumeAfterStop("session-lease-foreign"),
    );
    let outcome: Awaited<ReturnType<typeof result.current>> | undefined;
    await act(async () => {
      outcome = await result.current();
  ...
  test("a post-stop block with no dead lease to break is not retried either", async () => {
    resumeAfterStopMock.mockResolvedValue("blocked");
    getLiveStartBlockReasonMock.mockReturnValue("post_stop_processing");
    abandonCaptureRecoveryFinalizationMock.mockReturnValue(false);

    const { result } = renderHook(() =>
      useResumeAfterStop("session-lease-alive"),
    );
    await act(async () => {
      await result.current();
    });

    expect(abandonCaptureRecoveryFinalizationMock).toHaveBeenCalledOnce();
    expect(resumeAfterStopMock).toHaveBeenCalledOnce();
  });

  test("a finished resume does not block the next one", async () => {
    const { result } = renderHook(() => useResumeAfterStop("session-serial"));
  ...
  test("useResumeAfterStop reports blocked when the start bails out silently", async () => {
    canStartLiveSessionMock.mockReturnValue(false);
    getLiveStartBlockReasonMock.mockReturnValue(null);
    ...
    expect(startMock).not.toHaveBeenCalled();
    expect(outcome).toEqual({ status: "blocked", reason: "start_failed" });
  });

  test("a failing marker retirement does not abort the resume", async () => {
    loadCaptureLifecycleMarkerMock.mockResolvedValue({
      version: 1,
      sessionId: "session-1",
      transcriptId: "transcript-stranded",
    ...
    clearCaptureLifecycleMarkerMock.mockRejectedValue(
  ...
  test("two resumes at once do one start and retire the marker once", async () => {
    loadCaptureLifecycleMarkerMock.mockResolvedValue({
      version: 1,
      sessionId: "session-1",
      transcriptId: "transcript-stranded",
    ...
    const releaseResume: Array<() => void> = [];
    resumeAfterStopMock.mockImplementation(
      () =>
        new Promise<"started">((resolve) => {
          releaseResume.push(() => resolve("started"));
        }),
    );
    const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});

    const { result } = renderHook(() => useResumeAfterStop("session-1"));
    let outcomes: Awaited<ReturnType<typeof result.current>>[] = [];
    await act(async () => {
      const first = result.current();
      const second = result.current();
      await Promise.resolve();
      for (const release of releaseResume) {
        release();
      }
    ...
    expect(startMock).toHaveBeenCalledOnce();
    expect(outcomes[0]).toEqual({ status: "started" });
    expect(outcomes[1]).toEqual({ status: "started" });
    consoleWarn.mockRestore();
  });

  test("a resume during the finalization frees the lease instead of leaving it held", async () => {
    attachLiveSessionMock.mockResolvedValue("inactive");
    loadCaptureLifecycleMarkerMock.mockResolvedValue({
      version: 1,
      sessionId: "session-1",
    ...
    // Seen by the entry check and the pre-finalization check, then true for
    // every later look: the resume lands while recoverStopped is in flight.
    isCaptureRecoveryCancelledMock
      .mockReturnValueOnce(false)
      .mockReturnValueOnce(false)
  ...
  test("a resume that lands while recoverStopped runs releases the lease on the way out", async () => {
    attachLiveSessionMock.mockResolvedValue("inactive");
    // The marker survives the repair, so without the give-way check the run
    // would take the failure exit and walk off holding the lease.
    loadCaptureLifecycleMarkerMock.mockResolvedValue({
      version: 1,
      sessionId: "session-1",
      transcriptId: "transcript-before-reload",
    ...
    let resumeArrived = false;
    isCaptureRecoveryCancelledMock.mockImplementation(() => resumeArrived);
    audioPathMock.mockImplementation(async () => {
      // The user hits "keep recording" while the repair is under way.
      resumeArrived = true;
      return { status: "ok", data: "/tmp/existing-session.mp3" };
    });
    const consoleError = vi
      .spyOn(console, "error")
      .mockImplementation(() => {});
  ...
  test("a cancelled capture recovery never finalizes the session behind the resume", async () => {
    isCaptureRecoveryCancelledMock.mockReturnValue(true);
    attachLiveSessionMock.mockResolvedValue("inactive");
