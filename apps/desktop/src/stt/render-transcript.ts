...
/**
 * Startzeit eines Transkript-Blocks als mm:ss (ab einer Stunde h:mm:ss).
 *
 * Wird in den Zusammenfassungs-Prompt gegeben. Seit die Block-Bildung an den
 * letzten Block DESSELBEN Kanals anhaengt, ueberlappen sich Bloecke in der
 * Zeit: waehrend die eine Seite spricht, wirft die andere ein. Ohne Zeitmarke
 * liest ein Sprachmodell die Abfolge als zwei Monologe und erfindet eine
 * Kausalitaet, die es im Gespraech nie gab.
 */
export function formatSegmentStartLabel(startMs: number): string | null {
  if (!Number.isFinite(startMs) || startMs < 0) {
    return null;
  }

  const gesamtSekunden = Math.floor(startMs / 1000);
  ...
  const zweistellig = (wert: number) => wert.toString().padStart(2, "0");

  return stunden > 0
    ? `${stunden}:${zweistellig(minuten)}:${zweistellig(sekunden)}`
    : `${zweistellig(minuten)}:${zweistellig(sekunden)}`;
}
