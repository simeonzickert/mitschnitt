...
import { useResumeAfterStop, useStartListening } from "./useStartListening";

import { listenerStore } from "~/store/zustand/listener/instance";

const LISTENER_CONTROL_EVENT = "anlg:listener-control";

type ListenerControlAction = "start" | "stop" | "resume";
...
function MainListenerControlRequestRunner({
  ...
  const resumeAfterStop = useResumeAfterStop(request.sessionId);
  const stop = useListener((state) => state.stop);
  const startListeningRef = useRef(startListening);
  startListeningRef.current = startListening;
  const resumeAfterStopRef = useRef(resumeAfterStop);
  resumeAfterStopRef.current = resumeAfterStop;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        console.error("[listener] listener control request failed", {
          action: request.action,
          sessionId: request.sessionId,
          error,
        });
      } finally {
        // The queue only advances here. A request that threw and never
        // reported back would freeze every later request of this window.
        if (!cancelled) {
