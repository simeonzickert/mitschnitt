...
export function LiveCaptureRecovery() {
  ...
  // A deliberate resume takes the session over; any run still pending here
  // would otherwise keep retrying and finalize the session behind its back.
  useEffect(
    () =>
      subscribeCaptureRecoveryCancelled((sessionId) => {
        setRecoveryTokens((current) => {
