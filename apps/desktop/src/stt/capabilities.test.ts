...
describe("isSupportedLocalSttModel", () => {
  ...
  test("rejects cloud, local LLM, and removed local model ids", () => {
    ...
    // Die drei Argmax-Modelle sind am 01.09.2026 entfallen. Sie duerfen nicht
    // mehr als lokal durchgehen -- sonst schickt die Oberflaeche sie an ein
    // Rust-Kommando, dessen Enum sie nicht mehr kennt.
    expect(isSupportedLocalSttModel("am-parakeet-v2")).toBe(false);
    expect(isSupportedLocalSttModel("am-parakeet-v3")).toBe(false);
    expect(isSupportedLocalSttModel("am-whisper-large-v3")).toBe(false);
  });
});
...
describe("isOnDeviceSttModel", () => {
  ...
  /// Whisper muss on-device zaehlen, sonst startet `syncLocalSttServer` den
  /// lokalen Server nie und die Auswahl bliebe folgenlos.
  test("counts Whisper as on-device so its local server gets started", () => {
    expect(isOnDeviceSttModel("whispercpp", "QuantizedLargeTurbo")).toBe(true);
    expect(isOnDeviceSttModel("whispercpp", "soniqo-parakeet-batch")).toBe(
      false,
    );
    expect(isConfiguredSttModel("whispercpp", "QuantizedLargeTurbo")).toBe(
      true,
    );
    expect(isConfiguredSttModel("whispercpp", "apple-speech")).toBe(false);
  });
});
...
describe("isRealtimeLocalModel", () => {
  ...
  test("rejects batch-only local models", () => {
    ...
    expect(isRealtimeLocalModel("QuantizedLargeTurbo")).toBe(false);
  });
});

describe("isConfiguredSttModel", () => {
  // Opus 5 (Review 02.09.2026, C3 v): "anarlog" und "hyprnote" sind die
  // Kennungen der Gegenstelle des Originals. Der Anbieter existiert in
  // diesem Fork nicht -- nichts unter ihm ist eingerichtet, auch kein
  // lokales Modell: das zieht normalizeStoredSttSelection beim Start zu
  // seinem heutigen Anbieter um. Vorher galt "anarlog"/"cloud" als
  // eingerichtet, und "hyprnote" fiel als unbekannter Anbieter auf `true`.
  test.each([
    ["anarlog", "cloud"],
    ["hyprnote", "cloud"],
    ["anarlog", "soniqo-qwen3-small"],
    ["anarlog", "removed-local-model"],
  ])("treats %s / %s as not configured", (provider, model) => {
    expect(isConfiguredSttModel(provider, model)).toBe(false);
  });
...
describe("isDesktopLocalSttAvailable", () => {
