...
vi.mock("./capture-recovery-requests", () => ({
  ...
  subscribeCaptureRecoveryCancelled: mocks.subscribeCaptureRecoveryCancelled,
}));
...
beforeEach(() => {
  ...
  mocks.cancellationHandler = undefined;
  mocks.subscribeCaptureRecoveryCancelled.mockImplementation(
    (handler: (sessionId: string) => void) => {
      mocks.cancellationHandler = handler;
      return () => {
        mocks.cancellationHandler = undefined;
      };
    },
...
test("drops a pending recovery run once that session was cancelled", async () => {
  vi.useFakeTimers();
  const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
  mocks.getCaptureSnapshot.mockResolvedValue({
    status: "ok",
  ...
  const resume = vi.fn().mockResolvedValue("error");
  mocks.resumeBySession.set("session-cancelled", resume);
  ...
  act(() => {
    mocks.cancellationHandler?.("session-cancelled");
  });
